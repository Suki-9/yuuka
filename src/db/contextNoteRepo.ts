...
export function getContextNote(userId: string, botId: string): string {
  const db = getDb();
  const row = db
    .prepare("SELECT content FROM context_notes WHERE user_id = ? AND bot_id = ?")
    .get(userId, botId) as { content: string } | undefined;
  return row?.content ?? "";
}

export function getContextNoteUpdatedAt(userId: string, botId: string): string | null {
  const db = getDb();
  const row = db
    .prepare("SELECT updated_at FROM context_notes WHERE user_id = ? AND bot_id = ?")
 ...
 */
export function setContextNote(userId: string, botId: string, content: string): void {
 ...
 * @returns 追記後の全文
 */
export function appendContextNote(userId: string, botId: string, line: string): string {
