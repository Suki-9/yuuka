...
export function getContextNote(userId: string): string {
  const db = getDb();
  const row = db
    .prepare("SELECT content FROM context_notes WHERE user_id = ?")
    .get(userId) as { content: string } | undefined;
  return row?.content ?? "";
}

export function getContextNoteUpdatedAt(userId: string): string | null {
  const db = getDb();
  const row = db
    .prepare("SELECT updated_at FROM context_notes WHERE user_id = ?")
 ...
 */
export function setContextNote(userId: string, content: string): void {
  ...
  const db = getDb();
  db.prepare(
    `INSERT INTO context_notes (user_id, content, updated_at)
     VALUES (?, ?, datetime('now', 'localtime'))
     ON CONFLICT(user_id) DO UPDATE SET content = excluded.content, updated_at = datetime('now', 'localtime')`
  ).run(userId, content);
}
 ...
 * @returns 追記後の全文
 */
export function appendContextNote(userId: string, line: string): string {
  const trimmed = line.trim();
  if (!trimmed) {
    throw new Error("追記する内容が空です");
  }
  const current = getContextNote(userId);
