...
export function getExpenseById(
	...
	return db
		.prepare(
			"SELECT * FROM expenses WHERE user_id = ? AND bot_id = ? AND id = ?",
		)
...
export function getMonthlyTotal(
	...
	const row = db
       ...
       WHERE user_id = ? AND bot_id = ? AND type = ? AND date LIKE ?`,
		...
		.get(userId, botId, type, `${monthPrefix(year, month)}%`) as {
		total: number;
	};
	return row.total;
}
...
export function getMonthlyTrend(
	...
	return labels.map(
		(month) => byMonth.get(month) ?? { month, income: 0, expense: 0 },
	);
}
...
export function deleteBudgetLimit(
	...
	const result = db
		.prepare(
			"DELETE FROM budget_limits WHERE user_id = ? AND bot_id = ? AND category = ?",
		)
