...
export function addEntry(
  ...
  const db = getDb();
  const result = db
    .prepare("INSERT INTO clipboard_entries (user_id, bot_id, content, expires_at) VALUES (?, ?, ?, ?)")
...
export function deleteEntry(userId: string, botId: string, id: number): boolean {
  const db = getDb();
  const result = db
    .prepare("DELETE FROM clipboard_entries WHERE user_id = ? AND bot_id = ? AND id = ?")
    .run(userId, botId, id);
