 ...
 */
export function addEntry(userId: string, content: string, expiresAt: string | null): ClipboardEntry {
  const db = getDb();
  const result = db
    .prepare("INSERT INTO clipboard_entries (user_id, content, expires_at) VALUES (?, ?, ?)")
    .run(userId, content, expiresAt);
  return db
...
export function listEntries(userId: string): ClipboardEntry[] {
  const db = getDb();
  return db
    .prepare(
    ...
    .all(userId) as ClipboardEntry[];
}

export function deleteEntry(userId: string, id: number): boolean {
  const db = getDb();
  const result = db
    .prepare("DELETE FROM clipboard_entries WHERE user_id = ? AND id = ?")
    .run(userId, id);
