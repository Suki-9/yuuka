...
export function grantCredentialToBot(botId: string, ownerId: string, serviceName: string): void {
  getDb()
    .prepare(
...
export function revokeCredentialFromBot(botId: string, ownerId: string, serviceName: string): void {
  getDb()
    .prepare(
...
export function listBotIdsForCredential(ownerId: string, serviceName: string): string[] {
  return (
    getDb()
      .prepare("SELECT bot_id FROM bot_credential_access WHERE owner_id = ? AND service_name = ?")
      .all(ownerId, serviceName) as { bot_id: string }[]
  ).map((r) => r.bot_id);
}

/** Botが利用を許可されている認証情報名一覧（owner所有分）。 */
export function listCredentialNamesForBot(botId: string, ownerId: string): string[] {
  return (
    getDb()
      .prepare("SELECT service_name FROM bot_credential_access WHERE bot_id = ? AND owner_id = ?")
...
export function deleteAllGrantsForCredential(ownerId: string, serviceName: string): void {
  getDb()
    .prepare("DELETE FROM bot_credential_access WHERE owner_id = ? AND service_name = ?")
