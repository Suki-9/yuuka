...
export function addGoogleAccount(
	...
	const existing = input.email
		? (db
				.prepare(
					"SELECT * FROM user_google_accounts WHERE user_id = ? AND email = ?",
				)
...
export function getGoogleAccountById(
	id: number,
): UserGoogleAccount | undefined {
	return getDb()
		.prepare("SELECT * FROM user_google_accounts WHERE id = ?")
...
export function listGoogleAccountsSafe(
	userId: string,
): UserGoogleAccountSafe[] {
	return listGoogleAccounts(userId).map(toSafe);
}

/** owner の primary アカウント（バックアップ・秘書フォールバックで使用）。 */
export function getPrimaryGoogleAccount(
	userId: string,
): UserGoogleAccount | undefined {
	return getDb()
		.prepare(
...
export function setPrimaryGoogleAccount(
	userId: string,
	accountId: number,
): boolean {
	const db = getDb();
	...
	const tx = db.transaction(() => {
		db.prepare(
			"UPDATE user_google_accounts SET is_primary = 0 WHERE user_id = ?",
		).run(userId);
		db.prepare(
...
export function updateGoogleCalendars(
	accountId: number,
	calendars: string[],
): void {
	getDb()
 ...
 */
export function deleteGoogleAccount(
	userId: string,
	accountId: number,
): boolean {
	const db = getDb();
	...
	const tx = db.transaction(() => {
		...
		if (acct.is_primary === 1) {
			...
			if (next) {
				db.prepare(
					"UPDATE user_google_accounts SET is_primary = 1 WHERE id = ?",
				).run(next.id);
			}
		}
	});
 ...
 */
export function getAccountForBot(
	botId: string,
 ...
 */
export function setBotGoogleAccount(
	botId: string,
