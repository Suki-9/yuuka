...
export function getBriefingConfig(
	userId: string,
	botId: string,
): BriefingConfigRecord | undefined {
	const db = getDb();
	return db
	...
	db.prepare(
     ...
     ON CONFLICT(user_id, bot_id) DO UPDATE SET
       ...
       updated_at = datetime('now', 'localtime')`,
		...
		input.weatherLat !== undefined
			? input.weatherLat
			: (current?.weather_lat ?? null),
		input.weatherLng !== undefined
			? input.weatherLng
			: (current?.weather_lng ?? null),
		input.locationName !== undefined
			? input.locationName
			: (current?.location_name ?? null),
		JSON.stringify(input.newsFeeds ?? parseJsonArray(current?.news_feeds)),
		JSON.stringify(
			input.newsKeywords ?? parseJsonArray(current?.news_keywords),
		),
	);
...
export function listEnabledBriefingConfigsAcrossUsers(): BriefingConfigRecord[] {
	const db = getDb();
	return db
		.prepare("SELECT * FROM briefing_configs WHERE enabled = 1")
		.all() as BriefingConfigRecord[];
}
