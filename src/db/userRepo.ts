...
export function createUser(discordId: string, username: string, password: string): UserRecord {
  ...
  const runTx = db.transaction(() => {
    ...
    const role = isFirstUser || config.adminDiscordIds.includes(discordId) ? "admin" : "user";
 ...
 */
export function updatePassword(discordId: string, newPassword: string): boolean {
  const db = getDb();
...
export function updateUserRole(discordId: string, role: string): boolean {
  ...
  const db = getDb();
  const result = db
    .prepare("UPDATE users SET role = ?, updated_at = datetime('now', 'localtime') WHERE discord_id = ?")
    .run(role, discordId);
  return result.changes > 0;
}
...
export function isAdmin(discordId: string): boolean {
  const db = getDb();
  const row = db
    .prepare("SELECT 1 FROM users WHERE discord_id = ? AND role = 'admin' LIMIT 1")
    .get(discordId);
  return !!row;
}
 ...
 */
export function getUserGoogleConfig(discordId: string): UserGoogleConfig | null {
  const db = getDb();
  const row = db.prepare(`
  ...
  try {
    ...
    if (Array.isArray(parsed)) calendars = parsed.filter((c): c is string => typeof c === "string");
  } catch {
    // 不正なJSONは空リスト扱い
  }
...
export function getUserNotifyTarget(
  ...
  const db = getDb();
  const row = db
    .prepare("SELECT notify_target_type, notify_target_id FROM users WHERE discord_id = ?")
    .get(discordId) as { notify_target_type: string; notify_target_id: string | null } | undefined;
...
export function getUserSalt(discordId: string): string | null {
  const db = getDb();
  const row = db.prepare("SELECT salt FROM users WHERE discord_id = ?").get(discordId) as
    | { salt: string }
    | undefined;
 ...
 */
export function updateUserSettings(discordId: string, settings: UserSettingsUpdate): boolean {
  const sets: string[] = [];
  ...
  if (settings.notifyTargetType !== undefined) {
    if (settings.notifyTargetType !== "dm" && settings.notifyTargetType !== "channel") return false;
 ...
 */
export function getUserBackupConfig(discordId: string): UserBackupConfig | null {
  const db = getDb();
  const row = db.prepare(`
...
export function updateUserBackupSettings(
  discordId: string,
  settings: { enabled: boolean; intervalHours: number; generations: number; folderId?: string | null }
): boolean {
  ...
  const values: (string | number | null)[] = [settings.enabled ? 1 : 0, intervalHours, generations];
