 ...
 */
export function createEndpoint(userId: string, input: WebhookEndpointInput): WebhookEndpointRecord {
  const db = getDb();
...
export function getEndpointById(userId: string, id: number): WebhookEndpointRecord | undefined {
  const db = getDb();
  return db
...
export function getEndpointByToken(token: string): WebhookEndpointRecord | undefined {
  const db = getDb();
  return db
...
export function updateEndpoint(
  ...
  const result = db
    ...
    .run(
      ...
      input.notifyTargetId !== undefined ? input.notifyTargetId : current.notify_target_id,
      input.template !== undefined ? (input.template?.trim() || null) : current.template,
      input.filterKeyword !== undefined ? (input.filterKeyword?.trim() || null) : current.filter_keyword,
      input.createTodo !== undefined ? (input.createTodo ? 1 : 0) : current.create_todo,
      input.createReminder !== undefined ? (input.createReminder ? 1 : 0) : current.create_reminder,
...
export function listDeliveries(
  ...
  return db
    .prepare("SELECT * FROM webhook_deliveries WHERE user_id = ? ORDER BY id DESC LIMIT ?")
