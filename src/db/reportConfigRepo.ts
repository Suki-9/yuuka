...
export function getReportConfig(
	...
	botId: string,
	type: ReportType,
): ReportConfigRecord | undefined {
	const db = getDb();
	return db
		.prepare(
			"SELECT * FROM report_configs WHERE user_id = ? AND bot_id = ? AND type = ?",
		)
...
export function getReportConfigs(
	...
	return db
		.prepare(
			"SELECT * FROM report_configs WHERE user_id = ? AND bot_id = ? ORDER BY type ASC",
		)
