...
export function upsertReportConfig(
  ...
  const db = getDb();
  const current = getReportConfig(userId, type);

  db.prepare(
    `INSERT INTO report_configs (user_id, type, enabled, schedule_cron, target_type, target_id)
     VALUES (?, ?, ?, ?, ?, ?)
  ...
  return getReportConfig(userId, type)!;
}

export function getReportConfig(userId: string, type: ReportType): ReportConfigRecord | undefined {
  const db = getDb();
  return db
    .prepare("SELECT * FROM report_configs WHERE user_id = ? AND type = ?")
    .get(userId, type) as ReportConfigRecord | undefined;
}

export function getReportConfigs(userId: string): ReportConfigRecord[] {
  const db = getDb();
  return db
    .prepare("SELECT * FROM report_configs WHERE user_id = ? ORDER BY type ASC")
    .all(userId) as ReportConfigRecord[];
}
