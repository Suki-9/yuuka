 ...
 */
export function rescheduleRepeat(id: number, nextTriggerAt: string | Date): void {
  const db = getDb();
  db.prepare("UPDATE reminders SET trigger_at = ?, status = 'pending' WHERE id = ?").run(
    toDbDateTime(nextTriggerAt),
    id
  );
}
