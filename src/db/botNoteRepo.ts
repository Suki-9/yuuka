...
export function getBotUserNote(botId: string, userId: string): string {
  const db = getDb();
  const row = db
    .prepare("SELECT content FROM bot_context_notes WHERE bot_id = ? AND user_id = ?")
    .get(botId, userId) as { content: string } | undefined;
  return row?.content ?? "";
}

export function setBotUserNote(botId: string, userId: string, content: string): void {
...
export function appendBotUserNote(botId: string, userId: string, line: string): string {
...
export function getBotGuildNote(botId: string, guildId: string): string {
  const db = getDb();
  const row = db
    .prepare("SELECT content FROM bot_guild_notes WHERE bot_id = ? AND guild_id = ?")
    .get(botId, guildId) as { content: string } | undefined;
  return row?.content ?? "";
}

export function setBotGuildNote(botId: string, guildId: string, content: string): void {
...
export function appendBotGuildNote(botId: string, guildId: string, line: string): string {
  const trimmed = line.trim();
  if (!trimmed) throw new Error("追記する内容が空です");
