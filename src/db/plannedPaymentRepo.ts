...
export function getPlannedPaymentById(
	...
	return db
		.prepare(
			"SELECT * FROM planned_payments WHERE user_id = ? AND bot_id = ? AND id = ?",
		)
 ...
 */
export function listPendingDueWithinAcrossUsers(
	days: number,
): PlannedPaymentRecord[] {
	const db = getDb();
	return db
