 ...
 */
export function listServersGrantedToBotScoped(
	botId: string,
...
export function grantMcpToBot(
	...
	getDb()
		.prepare(
			"INSERT OR IGNORE INTO bot_mcp_access (bot_id, owner_id, mcp_server_id) VALUES (?, ?, ?)",
		)
		.run(botId, ownerId, serverId);
}

/** Botからサーバー利用許可を取り消す。 */
export function revokeMcpFromBot(
	botId: string,
	ownerId: string,
	serverId: number,
): void {
	getDb()
		.prepare(
			"DELETE FROM bot_mcp_access WHERE bot_id = ? AND owner_id = ? AND mcp_server_id = ?",
		)
		.run(botId, ownerId, serverId);
}
...
export function listBotIdsForServer(serverId: number): string[] {
	return (
		getDb()
			.prepare(
				"SELECT DISTINCT bot_id FROM bot_mcp_access WHERE mcp_server_id = ?",
			)
			.all(serverId) as {
			bot_id: string;
		}[]
...
export function listServerIdsForBot(botId: string, ownerId?: string): number[] {
	...
	const rows = (
		ownerId === undefined
			? db
					.prepare(
						"SELECT DISTINCT mcp_server_id FROM bot_mcp_access WHERE bot_id = ?",
					)
					.all(botId)
			: db
					.prepare(
						"SELECT mcp_server_id FROM bot_mcp_access WHERE bot_id = ? AND owner_id = ?",
					)
					.all(botId, ownerId)
...
export function isMcpGrantedToBot(botId: string, serverId: number): boolean {
	return !!getDb()
		.prepare(
			"SELECT 1 FROM bot_mcp_access WHERE bot_id = ? AND mcp_server_id = ? LIMIT 1",
		)
		.get(botId, serverId);
}
