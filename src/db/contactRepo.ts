...
export function isValidBirthday(birthday: string): boolean {
	return (
		/^\d{4}-\d{2}-\d{2}$/.test(birthday) || /^--\d{2}-\d{2}$/.test(birthday)
	);
}
...
export function getContactById(
	...
	return db
		.prepare(
			"SELECT * FROM contacts WHERE user_id = ? AND bot_id = ? AND id = ?",
		)
...
export function listContacts(userId: string, botId: string): ContactRecord[] {
	...
	return db
		.prepare(
			"SELECT * FROM contacts WHERE user_id = ? AND bot_id = ? ORDER BY name ASC",
		)
...
export function markBirthdayReminded(id: number, year: number): void {
	const db = getDb();
	db.prepare("UPDATE contacts SET birthday_reminded_year = ? WHERE id = ?").run(
		year,
		id,
	);
}
