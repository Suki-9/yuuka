...
export const webhookRoutes: RouteDef[] = [
  // ── 受信エンドポイント（公開 §3.13.2） ──
  {
    ...
    async handler(ctx) {
      if (!checkRateLimit(ctx.params.token)) {
        return sendJson(ctx.res, 429, { success: false, message: "rate limited" });
      }
      ...
      if (endpoint.enabled !== 1) {
        return sendJson(ctx.res, 410, { success: false, message: "endpoint disabled" });
      }
      ...
      processIncomingWebhook(endpoint, ctx.rawBody, signatureHeader).catch((err) => {
        console.error(`[Webhook] 受信処理で予期しないエラー (endpoint: ${endpoint.name}):`, err);
      });
    },
  },
  ...
  // ── 作成 ──
  {
    ...
    async handler(ctx) {
      ...
      const endpoint = createEndpoint(ctx.user!.discordId, {
        ...
        notifyTargetId:
          typeof ctx.body.notifyTargetId === "string" && ctx.body.notifyTargetId.trim()
            ...
            : null,
        template: typeof ctx.body.template === "string" ? ctx.body.template : null,
        filterKeyword: typeof ctx.body.filterKeyword === "string" ? ctx.body.filterKeyword : null,
      ...
      sendJson(ctx.res, 200, {
        success: true,
        endpoint: { ...toEndpointView(endpoint), url: buildHookUrl(endpoint.token) },
    ...
    async handler(ctx) {
      ...
      const ok = updateEndpoint(ctx.user!.discordId, id, {
        ...
        ...(ctx.body.notifyTargetId !== undefined
          ? { notifyTargetId: typeof ctx.body.notifyTargetId === "string" && ctx.body.notifyTargetId.trim() ? ctx.body.notifyTargetId.trim() : null }
          : {}),
        ...(ctx.body.template !== undefined ? { template: typeof ctx.body.template === "string" ? ctx.body.template : null } : {}),
        ...(ctx.body.filterKeyword !== undefined ? { filterKeyword: typeof ctx.body.filterKeyword === "string" ? ctx.body.filterKeyword : null } : {}),
        ...(ctx.body.createTodo !== undefined ? { createTodo: ctx.body.createTodo === true } : {}),
        ...(ctx.body.createReminder !== undefined ? { createReminder: ctx.body.createReminder === true } : {}),
      ...
      sendJson(ctx.res, 200, {
        success: ok,
        ...(fresh ? { endpoint: { ...toEndpointView(fresh), url: buildHookUrl(fresh.token) } } : {}),
        message: ok ? "Webhookエンドポイントを更新しました。" : "エンドポイントが見つかりません。",
      });
    },
  },
  ...
  // ── 削除 ──
  {
    ...
    async handler(ctx) {
      ...
      sendJson(ctx.res, 200, {
        success: ok,
        message: ok ? "Webhookエンドポイントを削除しました。" : "エンドポイントが見つかりません。",
      });
    },
  },
