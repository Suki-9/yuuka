...
function issueProxyToken(serverId: number, userId: string): string {
	...
	proxyTokens.set(token, {
		serverId,
		userId,
		expiresAt: now + PROXY_TOKEN_TTL_MS,
	});
	return token;
}

/** プロキシトークンを検証する（serverId 一致・未失効）。失効分はここでも掃除する。 */
function validateProxyToken(
	token: string,
	serverId: number,
): ProxyTokenEntry | null {
	const now = Date.now();
	sweepExpiredProxyTokens(now);
...
function toSafeView(server: McpServerRecord) {
	return {
		...
		tools: parseToolsCache(server).map((t) => ({
			name: t.name,
...
export const mcpRoutes: RouteDef[] = [
	...
	// v5: 登録/許可は統合管理ページで行う。ここは owner所有サーバー（許可付与の対象）＋システムレベル。
	{
		...
		async handler(ctx) {
			...
			const system = listSystemServers().map((s) => ({
				...toSafeView(s),
				granted_bot_ids: [] as string[],
			}));
	...
	// ── 追加（scope:"system" はAdminのみ §4.4.3） ──
	{
		...
		async handler(ctx) {
			...
			const endpointUrl =
				typeof ctx.body.endpointUrl === "string"
					? ctx.body.endpointUrl.trim()
					: "";
			const authCredential =
				typeof ctx.body.authCredential === "string"
					? ctx.body.authCredential
					: undefined;
			...
			if (!name || !endpointUrl) {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "name と endpointUrl は必須です。",
				});
			}
	...
	// ── ツール一覧の再取得 ──
	{
		...
		async handler(ctx) {
			...
			if (
				!server ||
				!canManage(server, user.discordId, user.role === "admin")
			) {
			...
			try {
				...
				sendJson(ctx.res, 502, {
					success: false,
					message: `tools/list の取得に失敗しました: ${(err as Error).message}`,
				});
			}
		},
	},
		...
		async handler(ctx) {
			...
			try {
				...
				rewritten = rewritten.replace(
					/var MCP_PATH = "[^"]*"/,
					`var MCP_PATH = "${proxyMcpUrl}"`,
				);
				if (rewritten === beforeMcpPathRewrite) {
					console.error(
						`[MCP] ${server.name}: ダッシュボードHTMLの MCP_PATH 書き換えに失敗しました（フォーマット不一致）`,
					);
					return sendFrameError(
						ctx.res,
						502,
				...
				if (rewritten === beforeTokenRewrite) {
					console.error(
						`[MCP] ${server.name}: ダッシュボードHTMLの tokenFromHash 書き換えに失敗しました（フォーマット不一致）`,
					);
					return sendFrameError(
						ctx.res,
						502,
				...
				if (akizakuraCss) {
					const akizakuraLinkRe =
						/<link\b[^>]*href="https:\/\/akizakura\.pages\.dev\/akizakura\.css"[^>]*>/i;
					if (akizakuraLinkRe.test(rewritten)) {
						rewritten = rewritten.replace(
							akizakuraLinkRe,
							`<style data-akizakura>${akizakuraCss}</style>`,
						);
					} else {
						console.error(
							`[MCP] ${server.name}: akizakura の <link> が見つからず inline 化できませんでした`,
						);
					}
				}
				...
				sendFrameError(
					ctx.res,
					502,
					`管理ページの取得に失敗しました: ${(err as Error).message}`,
				);
			}
		},
	},
	...
	// 使わないため、ACAO:null かつ Allow-Credentials 無しの安全な組合せで応答する。
	{
		...
		async handler(ctx) {
			...
			const reqHeaders =
				(ctx.req.headers["access-control-request-headers"] as
					| string
					| undefined) ??
	...
	// 不透明オリジンからのクロスオリジン fetch を許可するため ACAO:null を返す（Credentials 無し）。
	{
		...
		async handler(ctx) {
			...
			if (
				!canManage(server, tokenUser.discord_id, tokenUser.role === "admin")
			) {
				return sendJson(ctx.res, 403, {
					success: false,
					message: "このMCPサーバーを操作する権限がありません。",
				});
			}
			if (server.enabled !== 1) {
				return sendJson(ctx.res, 403, {
					success: false,
					message: "このMCPサーバーは無効化されています。",
				});
			}

			const accept =
				(ctx.req.headers["accept"] as string | undefined) ??
			...
			try {
				...
				if (!ctx.res.headersSent) {
					sendJson(ctx.res, 502, {
						success: false,
						message: `MCPサーバーへの接続に失敗しました: ${(err as Error).message}`,
					});
				}
				return;
			}
			...
			if (upstreamRes.body) {
				...
				try {
					...
					if (!controller.signal.aborted && !ctx.res.destroyed) {
						console.error(
							`[MCP proxy] ${server.name} のストリーム転送中にエラー:`,
