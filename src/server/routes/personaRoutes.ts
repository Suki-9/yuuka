...
export const personaRoutes: RouteDef[] = [
    ...
    async handler(ctx) {
      ...
      try {
        if (id != null && Number.isInteger(id)) {
          ...
          if (!ok) {
            return sendJson(ctx.res, 404, { success: false, message: "ペルソナが見つからないか、所有者ではありません。" });
          }
          sendJson(ctx.res, 200, { success: true, message: `ペルソナ「${name}」を更新しました。` });
        } else {
  ...
  // ── 削除 ──
  {
    ...
    async handler(ctx) {
      ...
      sendJson(ctx.res, 200, {
        success: ok,
        message: ok ? "ペルソナを削除しました。" : "ペルソナが見つからないか、所有者ではありません。",
      });
    },
  },
  ...
  // ── 適用（users.active_persona_id を更新） ──
  {
    ...
    async handler(ctx) {
      ...
      if (id != null) {
        if (!Number.isInteger(id)) {
          return sendJson(ctx.res, 400, { success: false, message: "id が不正です。" });
        }
  ...
  // ── 公開ペルソナの全文プレビュー（インポート判断用） ──
  {
    ...
    async handler(ctx) {
      ...
      if (!persona || persona.is_public !== 1) {
        return sendJson(ctx.res, 404, { success: false, message: "公開ペルソナが見つかりません。" });
      }
      sendJson(ctx.res, 200, {
        success: true,
  ...
  // ── インポート（独立コピー §4.1.3） ──
  {
    ...
    async handler(ctx) {
      ...
      if (!copied) {
        return sendJson(ctx.res, 404, { success: false, message: "公開ペルソナが見つかりません（非公開化された可能性があります）。" });
      }
      sendJson(ctx.res, 200, {
        success: true,
        persona: copied,
  ...
  // ── Botの推奨ペルソナ設定（Bot作成者のみ・is_public のみ可 §5.2.1） ──
  {
    ...
    async handler(ctx) {
      ...
      const personaId = ctx.body.personaId != null && ctx.body.personaId !== "" ? Number(ctx.body.personaId) : null;

      const bot = getBotById(botId);
      if (!bot || bot.user_id !== userId) {
        return sendJson(ctx.res, 403, { success: false, message: "Botの作成者のみが推奨ペルソナを設定できます。" });
      }
      ...
      if (personaId != null) {
        ...
        if (!persona || persona.is_public !== 1) {
          return sendJson(ctx.res, 400, { success: false, message: "公開（is_public）ペルソナのみ推奨ペルソナに設定できます。" });
        }
        setRecommendedPersona(botId, personaId);
        sendJson(ctx.res, 200, { success: true, message: `推奨ペルソナを「${persona.name}」に設定しました。` });
      } else {
        setRecommendedPersona(botId, null);
        sendJson(ctx.res, 200, { success: true, message: "推奨ペルソナを解除しました。" });
      }
    },
  },
  ...
  // ── Admin: マーケットプレイス管理（非公開化・削除 §5.3.2） ──
  {
    ...
    async handler(ctx) {
      ...
      if (ok) {
        addAuditLog(ctx.user!.discordId, "admin.persona_unpublish", `persona:${id}`);
      }
      sendJson(ctx.res, 200, {
        success: ok,
        message: ok ? "ペルソナを非公開化しました。" : "ペルソナが見つかりません。",
      });
    },
  },
  {
    ...
    async handler(ctx) {
      ...
      if (ok) {
        addAuditLog(ctx.user!.discordId, "admin.persona_delete", `persona:${id}`);
      }
      sendJson(ctx.res, 200, {
        success: ok,
