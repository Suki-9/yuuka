...
import { getBotGuildNote, setBotGuildNote, BOT_NOTE_MAX_LENGTH } from "../../db/botNoteRepo.js";
import {
  BOT_PRESETS,
  applyBotPreset,
  parseCapabilities,
...
function requireOwnedBot(ctx: RouteRequestCtx): BotRecord | null {
  ...
  if (!bot) {
    sendJson(ctx.res, 404, { success: false, message: "Botが見つかりません。" });
    return null;
  }
  if (bot.user_id !== ctx.user!.discordId && !isAdmin(ctx.user!.discordId)) {
    sendJson(ctx.res, 403, { success: false, message: "Botの作成者のみが設定を変更できます。" });
    return null;
  }
  return bot;
}
...
export const botAttributeRoutes: RouteDef[] = [
  ...
  // ── Bot属性（プリセット）の変更（owner / Admin のみ。即時反映 §4.1） ──
  {
    ...
    async handler(ctx) {
      ...
      if (bot.id === "system_default") {
        return sendJson(ctx.res, 400, { success: false, message: "デフォルトBotの属性は変更できません。" });
      }

      const presetInput = typeof ctx.body.preset === "string" ? ctx.body.preset : "";
      ...
      if (ok) {
        ...
        addAuditLog(ctx.user!.discordId, "bot.capabilities_change", bot.id, preset);
      }
      sendJson(ctx.res, 200, {
        success: ok,
        message: ok
  ...
  // ── Bot専用 Gemini APIキー（汎用モードで設定必須 §4.3.3） ──
  {
    ...
    async handler(ctx) {
      ...
      if (!apiKey.trim()) {
        ...
        addAuditLog(ctx.user!.discordId, "bot.gemini_key_change", bot.id, "cleared");
        return sendJson(ctx.res, 200, {
          success: true,
          message: "Bot専用APIキーを削除しました。キーが設定されるまでこのBotは応答しません。",
        });
      }

      if (apiKey.startsWith("••••")) {
        return sendJson(ctx.res, 200, { success: true, message: "APIキーは変更されていません。" });
      }

      const enc = encryptText(apiKey.trim());
      updateBotGeminiKey(bot.id, enc.encrypted, enc.iv, enc.authTag);
      addAuditLog(ctx.user!.discordId, "bot.gemini_key_change", bot.id, "updated");
      sendJson(ctx.res, 200, { success: true, message: "Bot専用のGemini APIキーを保存しました。" });
    },
  },
  ...
  // ── Bot単位ペルソナ（管理ページからのみ変更可能 §4.4） ──
  {
    ...
    async handler(ctx) {
      ...
      if (personaIdRaw !== null && personaIdRaw !== undefined && personaIdRaw !== "") {
        personaId = Number(personaIdRaw);
        if (!Number.isInteger(personaId)) {
          return sendJson(ctx.res, 400, { success: false, message: "personaId が不正です。" });
        }
        // 設定可能なのは owner 所有のもの、または公開ペルソナのみ（要件 §4.4）
        const persona = getPersonaById(personaId);
        if (!persona || (persona.owner_id !== bot.user_id && persona.is_public !== 1)) {
          return sendJson(ctx.res, 403, {
            success: false,
            message: "Bot作成者が所有するペルソナ、または公開ペルソナのみ設定できます。",
          });
        }
      }

      const ok = setBotPersona(bot.id, personaId);
      if (ok) addAuditLog(ctx.user!.discordId, "bot.persona_change", bot.id, personaId === null ? "cleared" : String(personaId));
      sendJson(ctx.res, 200, {
        success: ok,
  ...
  // ── 応答許可ギルドの管理（bot_guilds §4.3.3 / §6） ──
  {
    ...
    async handler(ctx) {
      ...
      const guildId = typeof ctx.body.guildId === "string" ? ctx.body.guildId.trim() : "";
      const action = ctx.body.action === "remove" ? "remove" : "add";
      if (!isSnowflake(guildId)) {
        return sendJson(ctx.res, 400, { success: false, message: "ギルドID（数字）を入力してください。" });
      }

      const ok = action === "add" ? addAllowedGuild(bot.id, guildId) : removeAllowedGuild(bot.id, guildId);
      addAuditLog(ctx.user!.discordId, "bot.guild_allow_change", bot.id, `${guildId}:${action}`);
      sendJson(ctx.res, 200, {
        success: true,
        guilds: listAllowedGuilds(bot.id),
        message: ok
  ...
  // ── 利用メンバーの管理（owner は管理UIから追加・削除 §4.3.3） ──
  {
    ...
    async handler(ctx) {
      ...
      const guildId = typeof ctx.body.guildId === "string" ? ctx.body.guildId.trim() : "";
      const userId = typeof ctx.body.userId === "string" ? ctx.body.userId.trim() : "";
      const action = ctx.body.action === "remove" ? "remove" : "add";
      if (!isSnowflake(guildId) || !isSnowflake(userId)) {
        return sendJson(ctx.res, 400, { success: false, message: "ギルドIDとユーザーID（数字）を入力してください。" });
      }
      ...
      addAuditLog(ctx.user!.discordId, action === "add" ? "bot.member_add" : "bot.member_remove", `${bot.id}:${guildId}:${userId}`);
      sendJson(ctx.res, 200, {
        success: true,
        members: listBotMembers(bot.id),
        message: ok
  ...
  // ── 共有ノート（ギルドノート）の閲覧・編集（owner §4.7） ──
  {
    ...
    async handler(ctx) {
      ...
      sendJson(ctx.res, 200, { success: true, content, max_length: BOT_NOTE_MAX_LENGTH });
    },
  },
  {
    method: "POST",
  ...
  },
  {
    ...
    auth: "admin",
    async handler(ctx) {
      const displayNames = (ctx.body.displayNames ?? {}) as Record<string, unknown>;
