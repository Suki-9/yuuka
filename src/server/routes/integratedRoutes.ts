...
function presetOf(bot: BotRecord): string {
	return presetIdForCapabilities(
		parseCapabilities(
...
export const integratedRoutes: RouteDef[] = [
	// ── 統合オーバービュー（1コールでページ全体を構成） ──
	{
		...
		async handler(ctx) {
			...
			const ownedBots = listBotsOwnedBy(userId).filter(
				(b) => b.id !== "system_default",
			);
			const botRecords: Array<{ bot: BotRecord; isSystemDefault: boolean }> =
				[];
			if (systemDefault)
				botRecords.push({ bot: systemDefault, isSystemDefault: true });
			for (const b of ownedBots)
				botRecords.push({ bot: b, isSystemDefault: false });
		...
		async handler(ctx) {
			...
			if (!bot || bot.user_id !== userId || botId === "system_default") {
				return sendJson(ctx.res, 403, {
					success: false,
					message: "このBotを操作する権限がありません。",
				});
			}
			if (isBotSuspended(botId)) {
				return sendJson(ctx.res, 409, {
					success: false,
					message: "このBotは管理者により停止されています。",
				});
			}
			if (!bot.discord_token_encrypted) {
				return sendJson(ctx.res, 409, {
					success: false,
					message: "Discordトークンが未設定です（Bot設定で登録してください）。",
				});
			}
	...
	//   ギルド会話は利用メンバー間で共有されるため「自分の会話」ではなく、ここでは対象外。
	{
		...
		async handler(ctx) {
			...
			if (
				!bot ||
				(botId !== "system_default" && !hasBotAccess(userId, botId))
			) {
				return sendJson(ctx.res, 403, {
					success: false,
					message: "このBotへのアクセス権がありません。",
				});
			}
	...
	// ── 利用許可トグル: 認証情報 ──
	{
		...
		async handler(ctx) {
			...
			const botId = typeof ctx.body.botId === "string" ? ctx.body.botId : "";
			const rawService =
				typeof ctx.body.serviceName === "string"
					? ctx.body.serviceName.trim().toLowerCase()
					: "";
			const granted = ctx.body.granted === true;
			if (!isGrantTargetBot(userId, botId)) {
				return sendJson(ctx.res, 403, {
			...
			if (
				!rawService ||
				!secretService.getDecryptedCredential(userId, rawService)
			) {
				return sendJson(ctx.res, 404, {
					success: false,
					message: "対象の認証情報が見つかりません。",
				});
			}
	...
	// ── 使用Googleアカウント割当（per-bot。system_default は発話者primaryのため対象外） ──
	{
		...
		async handler(ctx) {
			...
			const mode =
				typeof ctx.body.mode === "string" ? ctx.body.mode : "primary";
			const bot = getBotById(botId);
			if (!bot || bot.user_id !== userId || botId === "system_default") {
				return sendJson(ctx.res, 403, {
					success: false,
					message: "対象Botへの権限がありません（共有秘書は対象外）。",
				});
			}
			if (mode === "primary") {
			...
			const acct = Number.isInteger(accountId)
				? getGoogleAccountById(accountId)
				: undefined;
			if (!acct || acct.user_id !== userId) {
				return sendJson(ctx.res, 403, {
					success: false,
					message: "対象Googleアカウントの所有者ではありません。",
				});
			}
	...
	// ── Googleアカウント: primary変更 ──
	{
		...
		async handler(ctx) {
			...
			const ok =
				...
				setPrimaryGoogleAccount(userId, accountId);
	...
	// ── Googleアカウント: 削除 ──
	{
		...
		async handler(ctx) {
			...
			const accountId = Number(ctx.body.accountId);
			const ok =
				Number.isInteger(accountId) && deleteGoogleAccount(userId, accountId);
			if (ok) {
	...
	// ── Googleアカウント: 同期対象カレンダー更新 ──
	{
		...
		async handler(ctx) {
			...
			const calendars = Array.isArray(ctx.body.calendars)
				? (ctx.body.calendars as unknown[]).filter(
