...
export const adminRoutes: RouteDef[] = [
	// ── デフォルトBotのトークン更新 ──
	{
		...
		async handler(ctx) {
			const token =
				typeof ctx.body.token === "string" ? ctx.body.token.trim() : "";
			if (!token) {
	...
	// ── システム全体の統計 ──
	{
		...
		async handler(ctx) {
			...
			const botCount = db
				...
				.get() as { count: number };
			const suspendedBotCount = db
				.prepare("SELECT COUNT(*) as count FROM bots WHERE suspended = 1")
				.get() as { count: number };
			const inviteTotal = db
				.prepare("SELECT COUNT(*) as count FROM invite_codes")
				.get() as { count: number };
			const inviteUsed = db
				.prepare(
					"SELECT COUNT(*) as count FROM invite_codes WHERE used_by IS NOT NULL",
				)
				.get() as { count: number };
			const inviteAvailable = db
				.prepare(
					"SELECT COUNT(*) as count FROM invite_codes WHERE used_by IS NULL AND revoked_at IS NULL",
				)
				.get() as { count: number };
	...
	},
	{
		...
		async handler(ctx) {
			const privacyPolicyUrl =
				typeof ctx.body.privacyPolicyUrl === "string"
					? ctx.body.privacyPolicyUrl.trim()
					: "";
			const termsUrl =
				typeof ctx.body.termsUrl === "string" ? ctx.body.termsUrl.trim() : "";
	...
	},
	{
		...
		async handler(ctx) {
			...
			const targetUserId =
				typeof ctx.body.targetUserId === "string" ? ctx.body.targetUserId : "";
			const role = ctx.body.role;

			if (!targetUserId || !role) {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "targetUserId と role が必要です。",
				});
			}
			if (role !== "user" && role !== "admin") {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "role は 'user' または 'admin' のみ指定可能です。",
				});
			}
			// 自己降格防止
			if (targetUserId === adminId && role === "user") {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "自分自身の Admin 権限を解除することはできません。",
				});
			}
			...
			sendJson(ctx.res, 200, {
				success: true,
				message: `ユーザー ${targetUserId} のロールを ${role} に変更しました。`,
			});
		},
	},
	// ── ユーザーの強制削除（§5.3.2） ──
	{
		...
		async handler(ctx) {
			...
			sendJson(ctx.res, 200, {
				success: true,
				message: `ユーザー ${targetUserId} を削除しました（関連データも削除されました）。`,
			});
		},
	},
	...
	// ── 監査ログ閲覧（§5.3.2） ──
	{
		...
		async handler(ctx) {
			...
			const limit = Math.min(
				parseInt(ctx.url.searchParams.get("limit") || "200", 10),
				500,
			);
			const offset = Math.max(
				parseInt(ctx.url.searchParams.get("offset") || "0", 10),
				0,
			);
			sendJson(ctx.res, 200, {
				success: true,
	...
	},
	{
		...
		async handler(ctx) {
			...
			if (!success) {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "Botの停止処分に失敗しました。",
				});
			}
			addAuditLog(ctx.user!.discordId, "admin.bot_suspend", botId);
			console.log(
				`🚫 [Admin: ${ctx.user!.discordId}] Bot ${botId} を停止処分にしました`,
			);
			sendJson(ctx.res, 200, {
				success: true,
				message: `Bot ${botId} を停止処分にしました。Discordクライアントは停止されました。`,
			});
		},
	},
	{
		...
		async handler(ctx) {
			...
			if (!success) {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "停止処分の解除に失敗しました。",
				});
			}
			addAuditLog(ctx.user!.discordId, "admin.bot_unsuspend", botId);
			console.log(
				`✅ [Admin: ${ctx.user!.discordId}] Bot ${botId} の停止処分を解除しました`,
			);
			sendJson(ctx.res, 200, {
				success: true,
				message: `Bot ${botId} の停止処分を解除しました。所有者が再起動できるようになりました。`,
			});
		},
	},
	...
	},
	{
		...
		async handler(ctx) {
			const code =
				typeof ctx.body.code === "string" ? ctx.body.code.trim() : "";
			if (!code) {
				return sendJson(ctx.res, 400, {
					success: false,
