...
import { setSessionCookie, getSessionToken, getClientIp } from "../httpHelpers.js";
...
import { createPendingRegistration, verifyPendingRegistration } from "../../services/pendingRegistration.js";
...
const registerSendAttempts = new Map<string, { count: number; resetAt: number }>();
...
function allowRegisterSend(key: string): boolean {
  ...
  if (!a || a.resetAt <= now) {
    registerSendAttempts.set(key, { count: 1, resetAt: now + REGISTER_WINDOW_MS });
    return true;
  }
...
export const authRoutes: RouteDef[] = [
  ...
  // ── 初期セットアップ実行（最初のユーザー＝管理者登録） ──
  {
    ...
    async handler(ctx) {
      ...
      if (!isValidDiscordId(cleanDiscordId)) {
        return sendJson(ctx.res, 400, { success: false, message: "Discord ID の形式が不正です（17〜20桁の数字）。" });
      }
      if (cleanUsername.length > 64) {
        return sendJson(ctx.res, 400, { success: false, message: "ユーザーネームは64文字以内で入力してください。" });
      }
  ...
  // ── 新規登録（招待コード必須） ──
  {
    ...
    async handler(ctx) {
      ...
      if (!allowRegisterSend(sendKey)) {
        return sendJson(ctx.res, 429, { success: false, message: "確認コードの送信回数が上限に達しました。しばらくしてから再度お試しください。" });
      }
  ...
  // ── 登録の本人確認（DMで送ったワンタイムコードの検証 → 実ユーザー作成。G1対策） ──
  {
    ...
    async handler(ctx) {
      ...
      if (!discordId || !code) {
        return sendJson(ctx.res, 400, { success: false, message: "Discord ID と確認コードを入力してください。" });
      }
      const cleanDiscordId = discordId.trim();

      const result = verifyPendingRegistration(cleanDiscordId, String(code).trim());
      if (!result.ok) {
        const message =
    ...
    async handler(ctx) {
      ...
      if (user && verifyPassword(password, user.password_hash)) {
        ...
        if (!current || current.resetAt <= now) {
          loginAttempts.set(rlKey, { count: 1, resetAt: now + LOGIN_LOCKOUT_MS });
        } else {
          current.count += 1;
        }
