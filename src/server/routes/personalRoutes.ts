		...
		async handler(ctx) {
			...
			if (birthday && !isValidBirthday(birthday)) {
				return sendJson(ctx.res, 400, {
					success: false,
					message:
						"誕生日は YYYY-MM-DD 形式（年不明なら --MM-DD）で指定してください。",
				});
			}
			...
			const input = {
				...
				birthday,
				relationship:
					typeof ctx.body.relationship === "string" &&
					ctx.body.relationship.trim()
						? ctx.body.relationship.trim()
						: null,
				contactInfo:
					typeof ctx.body.contactInfo === "string" &&
					ctx.body.contactInfo.trim()
						? ctx.body.contactInfo.trim()
						: null,
				notes:
					typeof ctx.body.notes === "string" && ctx.body.notes.trim()
						? ctx.body.notes.trim()
						: null,
				tags: Array.isArray(ctx.body.tags)
					? (ctx.body.tags as unknown[]).map(String)
					: [],
			};
