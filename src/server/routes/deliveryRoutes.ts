...
export const deliveryRoutes: RouteDef[] = [
	...
	},
	{
		...
		async handler(ctx) {
			...
			const b = ctx.body;
			if (
				typeof b.schedule_cron === "string" &&
				!cron.validate(b.schedule_cron)
			) {
			...
			if (Array.isArray(b.news_feeds)) {
				const feeds = (b.news_feeds as unknown[])
					.map(String)
					.filter((s) => s.trim());
				...
				if (bad) {
					return sendJson(ctx.res, 400, {
						success: false,
						message: `不正なフィードURLです（http(s)の公開URLのみ指定できます）: ${bad}`,
					});
				}
			}
			...
			upsertBriefingConfig(userId, botId, {
				...
				...(b.target_id !== undefined
					? {
							targetId:
								typeof b.target_id === "string" && b.target_id.trim()
									? b.target_id.trim()
									: null,
						}
					: {}),
				...(b.weather_lat !== undefined
					? {
							weatherLat:
								b.weather_lat === null || b.weather_lat === ""
									? null
									: Number(b.weather_lat),
						}
					: {}),
				...(b.weather_lng !== undefined
					? {
							weatherLng:
								b.weather_lng === null || b.weather_lng === ""
									? null
									: Number(b.weather_lng),
						}
					: {}),
				...(b.location_name !== undefined
					? {
							locationName:
								typeof b.location_name === "string" && b.location_name.trim()
									? b.location_name.trim()
									: null,
						}
					: {}),
				...(Array.isArray(b.news_feeds)
					? { newsFeeds: (b.news_feeds as unknown[]).map(String) }
					: {}),
				...(Array.isArray(b.news_keywords)
					? { newsKeywords: (b.news_keywords as unknown[]).map(String) }
					: {}),
			});

			sendJson(ctx.res, 200, {
				success: true,
				message: "朝報の設定を保存しました。",
			});
		},
	},
	{
		...
		async handler(ctx) {
			...
			if (!config) {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "朝報がまだ設定されていません。先に設定を保存してください。",
				});
			}
	...
	},
	{
		...
		async handler(ctx) {
			...
			if (
				typeof ctx.body.schedule_cron === "string" &&
				!cron.validate(ctx.body.schedule_cron)
			) {
				return sendJson(ctx.res, 400, {
					success: false,
					message: "cron式が不正です。",
			...
			upsertReportConfig(userId, botId, type as ReportType, {
				...(ctx.body.enabled !== undefined
					...
					: {}),
				...(typeof ctx.body.schedule_cron === "string"
					? { scheduleCron: ctx.body.schedule_cron }
					: {}),
				...(ctx.body.target_type !== undefined
					? {
							targetType:
								ctx.body.target_type === "channel"
									? ("channel" as const)
									: ("dm" as const),
						}
					: {}),
				...(ctx.body.target_id !== undefined
					? {
							targetId:
								typeof ctx.body.target_id === "string" &&
								ctx.body.target_id.trim()
									? ctx.body.target_id.trim()
									: null,
						}
					: {}),
			...
			sendJson(ctx.res, 200, {
				success: true,
				message: `${type === "daily" ? "日報" : "週報"}の設定を保存しました。`,
			});
		},
	},
	{
