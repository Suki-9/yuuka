	...
	for (const reminder of due) {
		try {
			...
			if (!sent) {
				...
				console.warn(
					`⚠️ リマインド送信失敗のため再試行します (reminder #${reminder.id}, user: ${reminder.user_id})`,
				);
				continue;
			}
			...
			if (reminder.repeat_rule) {
				...
				try {
					...
					console.log(
						`🔁 繰り返しリマインド再設定: #${reminder.id} → ${formatDisplayDateTime(next.toISOString())} (user: ${reminder.user_id})`,
					);
				} catch (err) {
					// cron式が壊れている場合は無限再送を防ぐため送信済みで打ち切る
					console.error(
						`❌ repeat_rule の解釈に失敗したため単発扱いにします (reminder #${reminder.id}, rule: ${reminder.repeat_rule}):`,
						err,
					);
			...
			console.log(
				`🔔 リマインド送信: #${reminder.id} (user: ${reminder.user_id}, source: ${reminder.source})`,
			);
		} catch (err) {
	...
	for (const todo of todos) {
		try {
			...
			if (sent) {
				...
				console.log(
					`🔔 ToDo期限リマインド送信: todo #${todo.id} (user: ${todo.user_id})`,
				);
			} else {
				console.warn(
					`⚠️ ToDo期限リマインド送信失敗のため再試行します (todo #${todo.id}, user: ${todo.user_id})`,
				);
			}
		} catch (err) {
	...
	for (const schedule of schedules) {
		try {
			...
			if (sent) {
				...
				console.log(
					`🔔 予定リマインド送信: schedule #${schedule.id} (user: ${schedule.user_id})`,
				);
			} else {
				console.warn(
					`⚠️ 予定リマインド送信失敗のため再試行します (schedule #${schedule.id}, user: ${schedule.user_id})`,
				);
			}
		} catch (err) {
			console.error(
				`❌ 予定リマインド処理エラー (schedule #${schedule.id}):`,
				err,
			);
		}
	}
