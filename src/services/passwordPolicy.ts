...
function loadCommonPasswords(): Set<string> {
  ...
  for (const candidate of COMMON_PASSWORDS_CANDIDATES) {
    ...
    try {
      ...
      commonPasswords = set;
      console.log(`🔐 よく使われるパスワードリストを読み込みました（${set.size}件: ${candidate}）`);
      return commonPasswords;
    } catch (err) {
      console.error(`⚠️ パスワードリストの読み込みに失敗しました（${candidate}）:`, err);
    }
  }

  // リストが見つからない場合も登録自体は止めない（文字種・長さチェックのみ適用）
  console.warn("⚠️ common-passwords-10k.txt が見つかりません。一般的パスワードチェックをスキップします。");
...
export function validatePassword(password: string): PasswordValidationResult {
  ...
  if (loadCommonPasswords().has(password.toLowerCase())) {
    return {
      ok: false,
      reason: "よく使われるパスワードのため使用できません。別のパスワードを設定してください。",
    };
  }
