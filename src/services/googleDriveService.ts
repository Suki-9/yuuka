...
function getDriveClient(userId: string): drive_v3.Drive | null {
	...
	try {
		...
		console.error(
			"Google Drive 認証クライアントの初期化に失敗しました:",
			error,
		);
		return null;
	}
...
export async function uploadToGoogleDrive(
	...
	if (!drive) {
		throw new Error(
			"Google Driveクライアントが初期化されていません。Google OAuth連携を確認してください。",
		);
	}
	...
	try {
		...
		console.log(
			`Google Driveに新規ファイル(ID: ${fileId})を作成しました: ${fileName}`,
		);
...
export async function deleteDriveFile(
	...
	try {
		...
		console.error(
			`Google Driveのファイル削除に失敗しました (ID: ${fileId}):`,
			error,
		);
		return false;
	}
