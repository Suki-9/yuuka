...
async function fetchNewsItems(
	...
	for (const url of feedUrls.slice(0, 10)) {
		try {
			...
			console.warn(
				`[Briefing] RSSフィードの取得に失敗しました (${url}):`,
...
export async function runBriefingForUser(
	...
	if (config.weather_lat != null && config.weather_lng != null) {
		...
		if (weather && weather.length > 0) {
			const location =
				config.location_name || `${config.weather_lat}, ${config.weather_lng}`;
			const lines = weather.map((w, i) => {
				const dayLabel = i === 0 ? "今日" : "明日";
				const rain =
					w.precipitationProbability != null
						? ` / 降水確率 ${w.precipitationProbability}%`
						: "";
	...
	if (feeds.length > 0) {
		...
		if (items.length > 0) {
			...
			try {
				...
				console.warn(
					`[Briefing] ニュース要約に失敗しました (user: ${userId}):`,
					err,
				);
			}
...
async function tick(): Promise<void> {
	...
	for (const conf of configs) {
		...
		try {
			...
			console.error(
				`[Briefing] 朝報配信に失敗しました (user: ${conf.user_id}):`,
				err,
			);
		}
	}
...
export function startBriefingService(): void {
	...
	console.log(
		"🌅 朝報（天気・ニュース）サービスを開始しました（毎分cron判定）",
	);
}
