...
function collectReportData(
	...
	const now = new Date();
	const periodMs =
		type === "daily" ? 24 * 60 * 60 * 1000 : 7 * 24 * 60 * 60 * 1000;
	...
	const payments = db
       ...
       ORDER BY due_date ASC LIMIT 20`,
		...
		.all(userId, botId, fromDay, toDay) as {
		title: string;
		amount: number;
		due_date: string;
		status: string;
...
function buildFallbackText(data: ReportData): string {
	...
	for (const t of data.carryOverTodos.slice(0, 8))
		lines.push(`  ・${t.title}（期限: ${t.due_date}）`);
	if (data.schedules.length > 0) {
		lines.push(`📅 予定: ${data.schedules.length}件`);
		for (const s of data.schedules.slice(0, 8))
			lines.push(`  ・${s.title} (${s.start_at})`);
	}
	...
	lines.push(
		`💰 収支: 収入 ${formatCurrency(data.incomeTotal)} / 支出 ${formatCurrency(data.expenseTotal)}`,
	);
	return lines.join("\n");
}
...
export async function runReportForUser(
	...
	try {
		const source = [
			...
			...data.completedTodos.map(
				(t) => `- ${t.title} [${parseTodoTags(t).join("/")}]`,
			),
			`# 持ち越しタスク（${data.carryOverTodos.length}件）`,
			...data.carryOverTodos.map(
				(t) =>
					`- ${t.title}（期限: ${t.due_date}、優先度: ${t.priority ?? "未設定"}）`,
			),
			`# 予定（${data.schedules.length}件）`,
			...data.schedules.map((s) => `- ${s.title} (${s.start_at})`),
			`# 支払い予定`,
			...data.payments.map(
				(p) => `- ${p.title} ${p.amount}円 期日${p.due_date} 状態:${p.status}`,
			),
		...
		console.warn(
			`[Report] LLMサマリ生成に失敗しました (user: ${userId}):`,
			err,
		);
	}
	...
	const embed = new EmbedBuilder()
		...
		.addFields(
			{
				name: "✅ 完了",
				value: `${data.completedTodos.length}件`,
				inline: true,
			},
			{
				name: "📌 持ち越し",
				value: `${data.carryOverTodos.length}件`,
				inline: true,
			},
			{
				name: "💰 収支",
		...
		.setFooter({
			text: summary ? "Yuuka 自動生成レポート" : "Yuuka レポート（生データ）",
		})
		.setTimestamp();
...
async function tick(): Promise<void> {
	...
	for (const conf of configs) {
		...
		console.log(
			`📋 [Report] ${conf.type} レポートを生成します (user: ${conf.user_id})`,
		);
		try {
			await runReportForUser(conf.user_id, conf.bot_id, conf.type);
		} catch (err) {
			console.error(
				`[Report] レポート配信に失敗しました (user: ${conf.user_id}):`,
				err,
			);
		}
	}
