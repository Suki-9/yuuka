...
function ensureFontConfigured(): void {
	...
	try {
		...
		const found = cjkCandidates.find((c) =>
			families.some((f) => f === c || f.startsWith(c)),
		);
		if (found) {
			...
			const fuzzy = families.find((f) =>
				/CJK|JP|Gothic|Mincho|明朝|ゴシック/i.test(f),
			);
			if (fuzzy) {
				...
				console.warn(
					"[Chart] 日本語フォントが見つかりません。日本語ラベルが正しく描画されない可能性があります。",
				);
			}
		}
	} catch (err) {
