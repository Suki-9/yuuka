...
const memoryFallback = new Map<string, { actions: RecordedAction[]; expiresAt: number }>();
...
export async function recordFunctionCall(
  ...
  if (redis) {
    try {
      ...
      console.warn("[ActionRecorder] Redisへの記録に失敗しました（in-memoryへフォールバック）:", err);
    }
  }
  ...
  if (!entry || entry.expiresAt < now) {
    memoryFallback.set(userId, { actions: [action], expiresAt: now + TTL_SECONDS * 1000 });
  } else {
...
/** 直近の操作履歴を取得する（古い順） */
export async function getRecentActions(userId: string): Promise<RecordedAction[]> {
  const redis = getRedisClient();
  if (redis) {
    try {
