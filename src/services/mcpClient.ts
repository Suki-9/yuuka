...
function isSessionError(err: unknown): boolean {
	...
	return (
		/\bHTTP 40[04]\b/.test(msg) ||
		/session|not initialized|initializ/i.test(msg)
	);
}
...
export function buildAuthHeader(
	...
	try {
		...
		console.error(
			`[MCP] サーバー ${server.name} の認証情報の復号に失敗しました:`,
			err,
		);
		return {};
	}
 ...
 */
export async function probeMcpDashboard(
	server: McpServerRecord,
): Promise<boolean> {
	try {
		const res = await authedGet(
			server,
			`${mcpOrigin(server)}${DASHBOARD_ENABLE_PATH}`,
		);
...
export async function fetchAkizakuraCss(): Promise<string | null> {
	const now = Date.now();
	if (akizakuraCache && akizakuraCache.expiresAt > now)
		return akizakuraCache.css;
	const controller = new AbortController();
	const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
	try {
		const res = await fetch(AKIZAKURA_CSS_URL, {
			method: "GET",
			signal: controller.signal,
		});
		if (!res.ok) {
			console.error(
				`🔌 [MCP] akizakura.css の取得に失敗しました (HTTP ${res.status})`,
			);
		...
		console.error(
			`🔌 [MCP] akizakura.css の取得でエラー: ${(err as Error).message}`,
		);
		return akizakuraCache?.css ?? null;
	} finally {
		clearTimeout(timer);
...
export async function refreshToolsCache(serverId: number): Promise<number> {
	...
	console.log(
		`🔌 [MCP] ${server.name} のToolキャッシュを更新しました (${tools.length}件)`,
	);
	return tools.length;
}
