...
class CrawlerDaemon {
	...
	private start(): Promise<void> {
		...
		this.startingPromise = (async () => {
			...
			const proc = spawn(this.binPath, ["daemon"], {
			...
			proc.on("error", (err) => {
				...
				this.failAllPending(
					new Error(`Crawler daemon process error: ${err.message}`),
				);
			});
			...
			proc.on("exit", (code) => {
				console.error(
					`[Crawler Daemon] Process exited (code: ${code}), 次のリクエスト時に再起動します`,
				);
				rl.close();
				this.proc = null;
				this.startingPromise = null;
	...
	async send(
		...
		return new Promise<string>((resolve, reject) => {
			...
			proc.stdin!.write(
				JSON.stringify({ id, command, target, extra }) + "\n",
				(writeErr) => {
					if (writeErr && this.pending.has(id)) {
						clearTimeout(timer);
						this.pending.delete(id);
						reject(
							new Error(`Crawler daemon write failed: ${writeErr.message}`),
						);
					}
				},
			);
...
process.on("exit", () => {
	_crawlerDaemon?.shutdown();
});

async function runRustCrawler(
	command: string,
	...args: string[]
): Promise<string> {
...
function scheduleAutoClose(userId: string) {
	...
	session.autoCloseTimer = setTimeout(async () => {
		...
		if (Date.now() - s.lastInteractionTime >= AUTO_CLOSE_TIMEOUT_MS) {
			console.log(
				`[Interactive Browser] [User: ${userId}] 自動クローズタイマー作動 (5分間無操作)`,
			);
...
async function getInteractiveBrowser(
	userId: string,
	...
	const USER_DATA_DIR = path.resolve(
		process.cwd(),
		`data/browser_profiles/${userId}`,
	);

	console.log(
		`[Interactive Browser] [User: ${userId}] 新しいブラウザセッションを起動します...`,
	);
...
export async function closeInteractiveBrowser(userId: string): Promise<void> {
	...
	console.log(
		`[Interactive Browser] [User: ${userId}] ブラウザセッションをクローズしました。`,
	);
}
	...
	const rawMarkdown = await page.evaluate((interactive: boolean) => {
		...
		function isNoiseElement(el: any): boolean {
			...
			return noiseKeywords.some(
				(keyword) => id.includes(keyword) || className.includes(keyword),
			);
		}
		...
		function traverse(node: any, isPre = false): string {
			...
			switch (tagName) {
				...
				case "a": {
					...
					const yuukaId = interactive
						? node.getAttribute("data-yuuka-id") || ""
						: "";
...
export async function fetchCleanPageContent(
	...
	try {
		...
		console.warn(
			`[Rust Crawler] Fetch failed: ${err.message}. Falling back to Puppeteer...`,
		);
	}
	...
	if (!result) {
		try {
			...
			console.warn(
				`[Rust Crawler] JS fetch failed: ${err.message}. Falling back to Puppeteer...`,
			);
		}
	}
...
export async function takePageScreenshot(
	...
	try {
		...
		console.warn(
			`[Rust Crawler] Screenshot failed: ${err.message}. Falling back to Puppeteer...`,
		);
	}
...
export async function searchWeb(
	...
	try {
		...
		console.log(
			`[Rust Crawler] Successfully searched and found ${results.length} results.`,
		);
		return results;
	} catch (err: any) {
		console.warn(
			`[Rust Crawler] Search failed: ${err.message}. Falling back to Puppeteer...`,
		);
	}
			...
			const googleResults = await page.evaluate(() => {
				...
				elements.forEach((el: any) => {
					...
					const snippetEl = el.querySelector(
						"div.VwiC3b, span.aCOpbc, div.yD3zGc",
					);
					if (titleEl && anchor) {
						items.push({
...
export async function browserInteractiveOpen(
	...
	try {
		...
		console.warn(
			`[Interactive Browser] Navigation warn: ${err.message}, continuing...`,
		);
	}
	const title = await page.title();
...
export async function browserInteractiveClick(
	...
	if (match) {
		...
		try {
			...
			console.error(
				"[Interactive Browser] Smart click contains evaluation error:",
				evalErr,
			);
		}
		throw new Error(
	...
	try {
		...
		try {
			...
			console.error(
				"[Interactive Browser] Smart click fallback evaluation error:",
				evalErr,
			);
		}
		throw new Error(
			`要素またはテキスト "${selector}" のクリックに失敗しました: ${err.message}`,
		);
	}
}
...
export async function browserInteractiveType(
	...
	try {
		...
		try {
			...
			console.error(
				"[Interactive Browser] Smart type fallback evaluation error:",
				evalErr,
			);
		}
		throw new Error(
			`要素 "${selector}" へのテキスト入力に失敗しました: ${err.message}`,
		);
	}
}
