...
const KNOWN_CAPABILITIES: ReadonlySet<string> = new Set(["persona", "memory", "mcp", "secretary"]);

/** プリセットの内部ID（要件 §3.3: 内部IDは固定、表示名のみ変更可能） */
export type BotPresetId = "secretary" | "mcp_assistant";

/** プリセット定義（要件 §3.3） */
export const BOT_PRESETS: Record<BotPresetId, { capabilities: BotCapability[]; defaultDisplayName: string }> = {
  secretary: {
...
export const DEFAULT_CAPABILITIES_JSON = JSON.stringify(BOT_PRESETS.secretary.capabilities);
...
export function parseCapabilities(json: string | null | undefined): Set<BotCapability> {
  try {
    const parsed = JSON.parse(json || DEFAULT_CAPABILITIES_JSON);
    if (Array.isArray(parsed)) {
      const caps = parsed.map(String).filter((c) => KNOWN_CAPABILITIES.has(c)) as BotCapability[];
      return new Set(caps);
    }
  } catch {}
...
export function resolveBotCapabilities(botId: string): Set<BotCapability> {
  ...
  const caps = bot
    ? parseCapabilities((bot as unknown as { capabilities?: string }).capabilities)
...
export function botHasCapability(botId: string, cap: BotCapability | "core"): boolean {
...
export function getPresetDisplayName(presetId: BotPresetId): string {
  return getSystemSetting(presetDisplayNameKey(presetId), BOT_PRESETS[presetId].defaultDisplayName);
}

export function setPresetDisplayName(presetId: BotPresetId, displayName: string): void {
...
export function listPresets(): Array<{ id: BotPresetId; displayName: string; capabilities: BotCapability[] }> {
