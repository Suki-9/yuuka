...
async function incrementCounter(
	...
	if (redis) {
		try {
			...
			console.error(
				"⚠️ レート制限カウンタの更新に失敗しました（インメモリへフォールバック）:",
				err,
			);
		}
	}
...
export async function consumeRateLimit(
	...
	const userMinute = await incrementCounter(
		`mcp_rate:${botId}:${userId}:m`,
		60,
	);
	...
	const userDay = await incrementCounter(
		`mcp_rate:${botId}:${userId}:d:${day}`,
		25 * 60 * 60,
	);
	...
	const guildDay = await incrementCounter(
		`mcp_rate_guild:${botId}:${guildId}:d:${day}`,
		25 * 60 * 60,
	);
...
export function rateLimitMessage(
	exceeded: NonNullable<RateLimitResult["exceeded"]>,
): string {
	switch (exceeded) {
		case "user_minute":
