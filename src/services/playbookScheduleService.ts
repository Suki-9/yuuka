...
export function listSchedules(userId: string): PlaybookSchedule[] {
	...
	const rows = db
		.prepare(
			`SELECT * FROM playbook_schedules WHERE user_id = ? ORDER BY created_at DESC`,
		)
		.all(userId) as any[];
...
export function upsertSchedule(
	...
	const row = db
		.prepare(
			`SELECT * FROM playbook_schedules WHERE user_id = ? AND playbook_name = ?`,
		)
	...
	try {
		...
		if (!playbook) {
			finishRun(
				runId,
				"failed",
				`マクロ「${schedule.playbook_name}」が見つかりませんでした。`,
			);
			return;
		}
		...
		const result = await processMessage(schedule.bot_id, schedule.user_id, {
			text: prompt,
		});
...
export function startPlaybookScheduleService(): void {
	...
	console.log(
		`📅 マクロ（Playbook）スケジュールサービス開始 (${count}件のスケジュールを登録)`,
	);
}
