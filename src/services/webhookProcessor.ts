...
function verifyHmacSignature(
  ...
  let secret: string;
  try {
    secret = decryptText(endpoint.secret_encrypted, endpoint.secret_iv, endpoint.secret_tag);
  } catch (err) {
    console.error(`[Webhook] シークレットの復号に失敗しました (endpoint: ${endpoint.id}):`, err);
    return false;
  }
...
export async function processIncomingWebhook(
  ...
  if (endpoint.filter_keyword && !payloadText.includes(endpoint.filter_keyword)) {
  ...
  try {
    ...
    console.warn(`[Webhook] LLM解釈に失敗しました (endpoint: ${endpoint.name}):`, err);
  }
  ...
  const sent = await sendToUser(
    userId,
    { embeds: [embed] },
    { type: endpoint.notify_target_type, id: endpoint.notify_target_id ?? undefined }
  );
  ...
  if (endpoint.create_todo === 1) {
    try {
      ...
      console.error(`[Webhook] ToDo変換に失敗しました (endpoint: ${endpoint.name}):`, err);
    }
  }
  ...
  if (endpoint.create_reminder === 1) {
    try {
      ...
      console.error(`[Webhook] リマインド変換に失敗しました (endpoint: ${endpoint.name}):`, err);
    }
  }
