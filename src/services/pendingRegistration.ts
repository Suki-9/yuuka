 ...
 */
export function createPendingRegistration(discordId: string, data: PendingRegistration): string {
  const code = generateCode();
  pending.set(discordId, { ...data, code, expiresAt: Date.now() + TTL_MS, attempts: 0 });
  return code;
}

export type VerifyResult =
  | { ok: true; data: PendingRegistration }
  | { ok: false; reason: "not_found" | "expired" | "too_many_attempts" | "code_mismatch" };

/**
 * コードを検証する。成功時は保留データを返して当該保留を消費（削除）する。
 * 失敗時は試行回数を加算し、上限超過で保留を破棄する。
 */
export function verifyPendingRegistration(discordId: string, code: string): VerifyResult {
