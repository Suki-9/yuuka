...
export async function runBackup(userId: string): Promise<string> {
	...
	try {
		...
		try {
			...
			console.warn(
				`[Backup] 古い世代の削除に失敗しました (user: ${userId}):`,
				pruneErr,
			);
		}
		...
		console.log(
			`✅ [Backup] ユーザー ${userId} のバックアップ完了: ${result?.url}`,
		);
		return result?.url || "";
	} catch (err) {
		console.error(
			`❌ [Backup] ユーザー ${userId} のバックアップ処理中にエラーが発生しました:`,
			err,
		);
		throw err;
	} finally {
...
async function backupTick(): Promise<void> {
	...
	for (const userId of userIds) {
		try {
			...
			const lastRun = conf.lastRunAt
				? new Date(conf.lastRunAt.replace(" ", "T")).getTime()
				: 0;
			if (Date.now() - lastRun < intervalMs) continue;

			console.log(
				`⏰ [Backup] ユーザー ${userId} の定期バックアップを開始します...`,
			);
			await runBackup(userId);
		} catch (err) {
			console.error(
				`❌ [Backup] ユーザー ${userId} の定期バックアップに失敗しました:`,
				err,
			);
		}
	}
...
export function startBackupScheduler(): void {
	...
	console.log(
		"💾 バックアップスケジューラを開始しました（毎時15分に間隔判定）",
	);
}
