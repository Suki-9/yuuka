 ...
 */
export function toDiscordMarkdown(input: string, options: DiscordMarkdownOptions = {}): string {
...
function splitKeepingDelimiter(input: string, re: RegExp): Segment[] {
  ...
  for (const m of input.matchAll(re)) {
    ...
    if (start > lastIndex) segments.push({ text: input.slice(lastIndex, start), isMatch: false });
    segments.push({ text: m[0], isMatch: true });
    lastIndex = start + m[0].length;
  }
  if (lastIndex < input.length) segments.push({ text: input.slice(lastIndex), isMatch: false });
  return segments;
}

// ─── プレーン区間（コードブロック外）の変換 ─────────────────────────────────

function transformPlain(text: string, opts: Required<DiscordMarkdownOptions>): string {
...
function applyInline(text: string, opts: Required<DiscordMarkdownOptions>): string {
...
function convertTaskListLine(line: string): string {
  return line.replace(/^(\s*)[-*+]\s+\[([ xX/-])\]\s+/, (_m, indent: string, mark: string) => {
    let box: string;
    if (mark === " ") box = "⬜";
    else if (mark === "-" || mark === "/") box = "🔄"; // 進行中（拡張記法）
    else box = "✅";
    return `${indent}${box} `;
  });
}
...
function convertFootnoteDefLine(line: string): string {
  // 定義行 `[^id]: text` → `[※id] text`
  return line.replace(/^(\s*)\[\^([^\]]+)\]:\s*/, (_m, indent: string, id: string) => `${indent}[※${id}] `);
}
...
function convertInlineHtml(text: string): string {
  return text
    .replace(/<\s*(b|strong)\s*>([\s\S]*?)<\s*\/\s*\1\s*>/gi, (_m, _tag, inner: string) => `**${inner}**`)
    .replace(/<\s*(i|em)\s*>([\s\S]*?)<\s*\/\s*\1\s*>/gi, (_m, _tag, inner: string) => `*${inner}*`)
    .replace(/<\s*u\s*>([\s\S]*?)<\s*\/\s*u\s*>/gi, (_m, inner: string) => `__${inner}__`)
    .replace(/<\s*(mark|span)[^>]*>([\s\S]*?)<\s*\/\s*\1\s*>/gi, (_m, _tag, inner: string) => `**${inner}**`);
}
...
function isTableSeparator(line: string | undefined): boolean {
  ...
  if (cells.length === 0 || (cells.length === 1 && cells[0] === "")) return false;
...
function renderAsciiTable(header: string[], rows: string[][]): string {
  ...
  const renderRow = (cells: string[]) =>
    Array.from({ length: colCount }, (_v, c) => padCell(cells[c] ?? "", widths[c])).join(" | ");
