...
const declarations: FunctionDeclaration[] = [
    ...
    parameters: {
      ...
      properties: {
        name: {
          ...
          description: "マクロの英数字ファイル名 (例: 'example_login', 'morning_check')",
        },
        title: {
          type: SchemaType.STRING,
          description: "マクロの分かりやすい日本語タイトル（呼び出し名。例: '朝の確認', 'サンプルサイトのログインと請求書取得'）",
        },
        keywords: {
          type: SchemaType.ARRAY,
          items: { type: SchemaType.STRING },
          description: "次回呼び出し時にヒットさせたい関連キーワードのリスト (例: ['朝', '確認', 'ニュース'])",
        },
        description: {
          type: SchemaType.STRING,
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: SchemaType.STRING,
          description: "検索したいキーワードや部分一致の文字列 (例: 'ログイン', 'でんき')。省略した場合は全マクロの一覧を返します。",
        },
      },
    },
  },
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        name: { type: SchemaType.STRING, description: "実行するマクロの英数字名（findPlaybooks の結果の name）" },
      },
      required: ["name"],
    },
  },
  {
    name: "deletePlaybook",
    description: "マクロ（Playbook）を削除します。削除前に対象のタイトルをユーザーに確認すること。",
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        name: { type: SchemaType.STRING, description: "削除するマクロの英数字名" },
      },
      required: ["name"],
    },
  },
...
const handlers: FunctionModule["handlers"] = {
  async savePlaybook(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const name = String(args.name ?? "").trim();
    const title = String(args.title ?? "").trim();
    const description = String(args.description ?? "").trim();
    const steps = String(args.steps ?? "").trim();
    const keywords = Array.isArray(args.keywords) ? (args.keywords as unknown[]).map(String) : [];
  ...
  async findPlaybooks(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    ...
    return JSON.stringify({ success: true, count: playbooks.length, playbooks });
  },
  ...
  async getRecentActionHistory(ctx: ToolContext): Promise<string> {
    ...
    if (actions.length === 0) {
      return JSON.stringify({
        success: true,
        count: 0,
        message: "直近の操作履歴がありません（履歴は2時間で揮発します）。ユーザーに手順を説明してもらい、説明ベースで登録してください。",
        actions: [],
      });
    }
    return JSON.stringify({
  ...
  async runPlaybook(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const name = String(args.name ?? "").trim();
    if (!name) {
    ...
    return JSON.stringify({
      success: true,
      message: "以下の手順に厳密に従って、各ツールを順番に実行してください。失敗した場合は中断して正直に報告すること。",
      playbook,
    });
  },

  async deletePlaybook(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const name = String(args.name ?? "").trim();
    if (!name) {
      return JSON.stringify({ success: false, message: "name は必須です。" });
    }
