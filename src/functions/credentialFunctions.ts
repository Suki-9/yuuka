    ...
    parameters: {
      ...
      properties: {
        service_name: {
          ...
          description: "サービス名（例: 'github', '社内ポータル'）。小文字に正規化して保存されます",
        },
        username: {
          type: SchemaType.STRING,
        ...
        password: {
          type: SchemaType.STRING,
          description: "パスワード（暗号化して保存されます。この値を以後の返信に繰り返さないこと）",
        },
        url: {
          type: SchemaType.STRING,
          description: "ログインページのURL（任意。ブラウザ自動ログイン時に開くページの参考になります）",
        },
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        service_name: {
          type: SchemaType.STRING,
          description: "更新対象のサービス名（listCredentialServices で確認可能）",
        },
        username: { type: SchemaType.STRING, description: "新しいユーザー名（変更する場合のみ指定）" },
        password: {
          type: SchemaType.STRING,
          description: "新しいパスワード（変更する場合のみ指定。この値を以後の返信に繰り返さないこと）",
        },
        url: {
          type: SchemaType.STRING,
          description: "新しいログインページURL（変更する場合のみ指定。空文字を指定するとURLを削除）",
        },
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        service_name: {
          type: SchemaType.STRING,
          description: "削除対象のサービス名（listCredentialServices で確認可能）",
        },
      },
      required: ["service_name"],
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        service_name: {
          type: SchemaType.STRING,
          description: "入力する認証情報のサービス名（listCredentialServices で確認可能）",
        },
        username_selector: {
          type: SchemaType.STRING,
          description:
...
const handlers: FunctionModule["handlers"] = {
  ...
  async listCredentialServices(ctx: ToolContext): Promise<string> {
    ...
    if (services.length === 0) {
      return JSON.stringify({
        success: true,
        message: "パスワードマネージャに登録済みの認証情報はありません。addCredential で登録できます。",
        services: [],
      });
    }
    const lines = services.map(
      (s) => `🔑 ${s.service_name}（ユーザー名: ${s.username}${s.url ? `、URL: ${s.url}` : ""}）`
    );
    return JSON.stringify({
      success: true,
  ...
  async addCredential(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
  ...
  async updateCredential(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const serviceName = asOptionalString(args.service_name);
    if (!serviceName) {
      return JSON.stringify({ success: false, message: "service_name を指定してください。" });
    }
    ...
    if (username === undefined && password === undefined && url === undefined) {
      return JSON.stringify({
        success: false,
        message: "更新する項目（username / password / url）を1つ以上指定してください。",
      });
    }

    try {
      const updated = secretService.updateCredential(ctx.userId, serviceName, { username, password, url });
      if (!updated) {
        return JSON.stringify({
          success: false,
      ...
      return JSON.stringify({
        success: false,
        message: sanitizeErrorMessage((err as Error).message, password ? [password] : []),
      });
    }
  ...
  async deleteCredential(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const serviceName = asOptionalString(args.service_name);
    if (!serviceName) {
      return JSON.stringify({ success: false, message: "service_name を指定してください。" });
    }
  ...
  async browserFillCredential(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const serviceName = asOptionalString(args.service_name);
    if (!serviceName) {
      return JSON.stringify({ success: false, message: "service_name を指定してください。" });
    }
    ...
    try {
      credential = secretService.getDecryptedCredential(ctx.userId, serviceName);
    } catch (err) {
    ...
    try {
      if (usernameSelector) {
        await browserInteractiveType(ctx.userId, usernameSelector, credential.username);
        filled.push("ユーザー名");
      }
      if (passwordSelector) {
        await browserInteractiveType(ctx.userId, passwordSelector, credential.password);
        filled.push("パスワード");
      }
    } catch (err) {
      ...
      return JSON.stringify({
        success: false,
        message:
          (filled.length > 0 ? `${filled.join("と")}の入力後にエラーが発生しました: ` : "入力に失敗しました: ") +
