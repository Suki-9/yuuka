...
import { CATEGORIES, type ExpenseRecord, type ExpenseType } from "../db/expenseRepo.js";
import * as plannedPaymentRepo from "../db/plannedPaymentRepo.js";
import type { PlannedPaymentRecord, PlannedPaymentStatus } from "../db/plannedPaymentRepo.js";
...
function planLine(plan: PlannedPaymentRecord): string {
  ...
  if (plan.status === "pending" && plan.due_date < todayYmd()) marks.push("⚠️期日超過");
  const statusLabel =
    plan.status === "settled" ? " ✅消込済" : plan.status === "cancelled" ? " ❌キャンセル" : "";
  const links: string[] = [];
  if (plan.linked_todo_id) links.push(`ToDo#${plan.linked_todo_id}`);
  if (plan.linked_reminder_id) links.push(`リマインド#${plan.linked_reminder_id}`);
  return (
...
const declarations: FunctionDeclaration[] = [
  // ── 収支記録（§3.4.1） ──
  {
    ...
    parameters: {
      ...
      properties: {
        amount: { type: SchemaType.NUMBER, description: "金額（円、1以上の整数）" },
        category: {
        ...
        type: {
          type: SchemaType.STRING,
          description: "収支区分: 'expense'（支出、デフォルト）| 'income'（収入）",
        },
        memo: { type: SchemaType.STRING, description: "メモ（店名・品目・用途など。任意）" },
        date: {
          type: SchemaType.STRING,
        ...
        time: {
          type: SchemaType.STRING,
          description: "時刻 (HH:MM:SS形式。レシートに記載がある場合など。任意)",
        },
        source: {
          type: SchemaType.STRING,
          description:
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        year: { type: SchemaType.NUMBER, description: "年（例: 2026。省略時は今年）" },
        month: { type: SchemaType.NUMBER, description: "月（1〜12。省略時は今月）" },
      },
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        type: {
          type: SchemaType.STRING,
          description: "集計対象: 'expense'（支出、デフォルト）| 'income'（収入）",
        },
      },
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        type: {
          type: SchemaType.STRING,
          description: "絞り込み: 'expense'（支出のみ）| 'income'（収入のみ）。省略時は両方",
        },
      },
    },
  },
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        category: { type: SchemaType.STRING, description: `予算を設定するカテゴリ（${CATEGORY_LIST} のいずれか）` },
        limit_amount: { type: SchemaType.NUMBER, description: "月次予算上限（円、1以上の整数）" },
      },
  ...
  },
  {
    ...
    description: "カテゴリの月次予算上限を削除します。「食費の予算設定を消して」などの依頼で呼び出してください。",
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        category: { type: SchemaType.STRING, description: "予算上限を削除するカテゴリ" },
      },
      required: ["category"],
    },
  },
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        title: { type: SchemaType.STRING, description: "支払いの概要（例: 家賃, Netflix, 自動車税）" },
        amount: { type: SchemaType.NUMBER, description: "予定金額（円、1以上の整数）" },
        category: { type: SchemaType.STRING, description: `カテゴリ（${CATEGORY_LIST} のいずれか）` },
        due_date: { type: SchemaType.STRING, description: "支払い期日 (YYYY-MM-DD形式)" },
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        plan_id: { type: SchemaType.NUMBER, description: "消込する支払い予定のID（#番号）" },
        expense_id: {
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        plan_id: { type: SchemaType.NUMBER, description: "キャンセルする支払い予定のID（#番号）" },
      },
      required: ["plan_id"],
    },
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        amount: { type: SchemaType.NUMBER, description: "実際の支払い金額（円、整数）" },
        category: { type: SchemaType.STRING, description: "実際の支払いのカテゴリ" },
        date: {
          type: SchemaType.STRING,
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        plan_id: { type: SchemaType.NUMBER, description: "ToDoを生成する支払い予定のID（#番号）" },
      },
      required: ["plan_id"],
    },
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        plan_id: { type: SchemaType.NUMBER, description: "リマインドを設定する支払い予定のID（#番号）" },
        days_before: {
          type: SchemaType.NUMBER,
          description: "期日の何日前に通知するか（0=期日当日の朝。デフォルト1）",
        },
      },
      required: ["plan_id"],
    },
...
const handlers: FunctionModule["handlers"] = {
  ...
  async addExpense(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const amount = asOptionalInt(args.amount);
    if (amount === undefined || amount <= 0) {
      return JSON.stringify({ success: false, message: "amount は1以上の整数（円）で指定してください。" });
    }
    ...
    if (date && !isYmd(date)) {
      return JSON.stringify({ success: false, message: "date は YYYY-MM-DD 形式で指定してください。" });
    }
    ...
    let budgetUsage: { limit_amount: number; spent: number; ratio_percent: number } | undefined;
  ...
  async getMonthlySummary(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    ...
    if (breakdown.length > 0) {
      ...
      for (const c of breakdown) {
        lines.push(`  ${c.category}: ${formatCurrency(c.total)} (${c.count}件)`);
      }
    }
  ...
  async getCategoryBreakdown(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const year = asOptionalInt(args.year);
    const month = asOptionalInt(args.month);
    const type: ExpenseType = args.type === "income" ? "income" : "expense";
    const typeLabel = type === "income" ? "収入" : "支出";
  ...
  // 直近の収支記録（§3.4.1）
  async listRecentExpenses(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
  ...
  async getBudgetLimits(ctx: ToolContext): Promise<string> {
    ...
    if (limits.length === 0) {
      return JSON.stringify({
        success: true,
        message: "予算上限が設定されているカテゴリはありません。setBudgetLimit で設定できます。",
        limits: [],
      });
    }
    ...
    const entries = limits.map((l) => {
      ...
      return { category: l.category, limit_amount: l.limit_amount, spent, ratio_percent: ratio };
    });

    const lines = entries.map((e) => {
      const warn = e.ratio_percent >= 100 ? " ⚠️超過" : e.ratio_percent >= 80 ? " ⚠️" : "";
  ...
  // 予算上限の設定（§3.4.1 予算管理）
  async setBudgetLimit(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const category = asOptionalString(args.category);
  ...
  // 予算上限の削除
  async deleteBudgetLimit(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const category = asOptionalString(args.category);
    if (!category) {
      return JSON.stringify({ success: false, message: "category を指定してください。" });
    }
    ...
    return JSON.stringify({ success: true, message: `${category} の予算上限を削除しました。` });
  },

  // 支払い予定の一覧（§3.4.3）
  async listPlannedPayments(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const statusArg = asOptionalString(args.status);
    const status: PlannedPaymentStatus | "all" =
      statusArg === "settled" || statusArg === "cancelled" || statusArg === "all"
        ? statusArg
        : "pending";
    ...
    if (plans.length === 0) {
      return JSON.stringify({
        success: true,
        message: status === "pending" ? "未消込の支払い予定はありません。" : "該当する支払い予定はありません。",
        plans: [],
      });
    }
  ...
  async addPlannedPayment(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const title = asOptionalString(args.title);
    if (!title) {
      return JSON.stringify({ success: false, message: "title（支払いの概要）を指定してください。" });
    }
    const amount = asOptionalInt(args.amount);
    if (amount === undefined || amount <= 0) {
      return JSON.stringify({ success: false, message: "amount は1以上の整数（円）で指定してください。" });
    ...
    if (!dueDate || !isYmd(dueDate)) {
      return JSON.stringify({ success: false, message: "due_date は YYYY-MM-DD 形式で指定してください。" });
    }
  ...
  async settlePlannedPayment(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    ...
    if (expenseId !== undefined) {
      ...
      if (found.type !== "expense") {
        return JSON.stringify({ success: false, message: `記録 #${expenseId} は収入のため消込に使えません。` });
      }
      expense = found;
    } else {
    ...
    if (plan.repeat_rule) {
      ...
      if (nextDue) {
        ...
        if (nextPlan) {
          notes.push(`🔁 次回の支払い予定 #${nextPlan.id}（期日: ${formatDate(nextDue)}）を自動生成しました。`);
        }
      } else {
        notes.push(`⚠️ repeat_rule の解釈に失敗したため次回予定は生成されませんでした (rule: ${plan.repeat_rule})。`);
      }
    }
  ...
  // 支払い予定のキャンセル
  async cancelPlannedPayment(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const planId = asOptionalInt(args.plan_id);
    if (planId === undefined) {
      return JSON.stringify({ success: false, message: "plan_id を指定してください。" });
    }
    ...
    if (plan.linked_todo_id) {
      notes.push(`紐付きToDo (#${plan.linked_todo_id}) は残っています。不要なら削除するかユーザーに確認してください。`);
    }
    if (plan.repeat_rule) {
  ...
  async findSettlementCandidates(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const amount = asOptionalInt(args.amount);
    if (amount === undefined || amount <= 0) {
      return JSON.stringify({ success: false, message: "amount は1以上の整数（円）で指定してください。" });
    }
  ...
  async linkPlannedPaymentTodo(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const planId = asOptionalInt(args.plan_id);
    if (planId === undefined) {
      return JSON.stringify({ success: false, message: "plan_id を指定してください。" });
    }
  ...
  async linkPlannedPaymentReminder(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const planId = asOptionalInt(args.plan_id);
    if (planId === undefined) {
      return JSON.stringify({ success: false, message: "plan_id を指定してください。" });
    }
    ...
    if (daysBefore < 0) {
      return JSON.stringify({ success: false, message: "days_before は0以上の整数で指定してください。" });
    }

    // 通知時刻: 期日の daysBefore 日前の朝9時
    const due = new Date(`${plan.due_date}T00:00:00`);
    if (Number.isNaN(due.getTime())) {
      return JSON.stringify({ success: false, message: `期日（${plan.due_date}）が日付として解釈できません。` });
    }
