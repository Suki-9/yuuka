...
const declarations: FunctionDeclaration[] = [
		...
		parameters: {
			...
			properties: {
				...
				latitude: {
					...
					description: "天気取得地点の緯度（例: 東京=35.68）",
				},
				longitude: {
					type: SchemaType.NUMBER,
					description: "天気取得地点の経度（例: 東京=139.77）",
				},
				location_name: {
					type: SchemaType.STRING,
					description: "地点の表示名（例: '東京'）",
				},
				add_news_feed: {
					type: SchemaType.STRING,
					description: "追加するRSSフィードURL",
				},
				remove_news_feed: {
					type: SchemaType.STRING,
					description: "削除するRSSフィードURL（部分一致可）",
				},
				news_keywords: {
					type: SchemaType.ARRAY,
					items: { type: SchemaType.STRING },
					description:
						"ニュースのキーワードフィルタ（全置換。空配列でフィルタ解除）",
				},
			},
		},
	},
	{
		...
		description:
			"朝報（天気・ニュース定期配信）の現在の設定と、日報・週報の配信設定を確認します。",
	...
	},
	{
		...
		parameters: {
			...
			properties: {
				type: {
					type: SchemaType.STRING,
					description: "'daily'（日報）または 'weekly'（週報）",
				},
				enabled: { type: SchemaType.BOOLEAN, description: "配信の有効/無効" },
				schedule_cron: {
					type: SchemaType.STRING,
					description:
						"配信時刻のcron式（例: 日報 '0 21 * * *' = 毎日21時、週報 '0 21 * * 0' = 毎週日曜21時）",
				},
			},
			required: ["type"],
		},
	},
	{
		name: "runBriefingNow",
		description:
			"朝報を今すぐテスト配信します。設定変更後の動作確認に使います。",
		parameters: { type: SchemaType.OBJECT, properties: {} },
	},
];
