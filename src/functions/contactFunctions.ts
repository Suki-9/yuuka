...
const declarations: FunctionDeclaration[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        name: { type: SchemaType.STRING, description: "氏名・呼称（例: '田中太郎', '佐藤さん'）" },
        birthday: {
          type: SchemaType.STRING,
          description: "誕生日。YYYY-MM-DD形式。年が不明な場合は --MM-DD 形式（例: '--05-03'）（任意）",
        },
        relationship: { type: SchemaType.STRING, description: "関係性（例: '同僚', '家族', '友人'）（任意）" },
        contact_info: { type: SchemaType.STRING, description: "電話・メール等の連絡先（任意）" },
        notes: { type: SchemaType.STRING, description: "自由記述メモ（好み・特記事項など）（任意）" },
        tags: {
          type: SchemaType.ARRAY,
          items: { type: SchemaType.STRING },
          description: "分類タグ（例: ['仕事', 'ミレニアム']）（任意。内容から適切に付与）",
        },
      },
      required: ["name"],
    },
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        query: { type: SchemaType.STRING, description: "検索キーワード（氏名・呼称・属性の一部）" },
      },
      required: ["query"],
    },
  },
  {
    name: "listContacts",
    description: "登録済みの連絡先一覧を取得します。「連絡先を見せて」などの依頼時に呼び出します。",
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        contact_id: { type: SchemaType.NUMBER, description: "更新する連絡先のID" },
        name: { type: SchemaType.STRING, description: "氏名（変更する場合のみ）" },
        birthday: { type: SchemaType.STRING, description: "誕生日 YYYY-MM-DD または --MM-DD（変更する場合のみ）" },
        relationship: { type: SchemaType.STRING, description: "関係性（変更する場合のみ）" },
        contact_info: { type: SchemaType.STRING, description: "連絡先情報（変更する場合のみ）" },
        notes: { type: SchemaType.STRING, description: "メモ（変更する場合のみ。既存メモに追記する場合は既存内容も含めた全文を渡す）" },
        tags: { type: SchemaType.ARRAY, items: { type: SchemaType.STRING }, description: "タグ（変更する場合のみ）" },
      },
      required: ["contact_id"],
    },
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        contact_id: { type: SchemaType.NUMBER, description: "削除する連絡先のID" },
      },
      required: ["contact_id"],
    },
  },
...
const handlers: FunctionModule["handlers"] = {
  addContact(ctx: ToolContext, args: Record<string, unknown>): string {
    ...
    if (birthday && !isValidBirthday(birthday)) {
      return JSON.stringify({
        success: false,
        message: "誕生日は YYYY-MM-DD 形式（年不明なら --MM-DD 形式）で指定してください。",
      });
    }
  ...
  updateContact(ctx: ToolContext, args: Record<string, unknown>): string {
    ...
    const birthday = args.birthday !== undefined ? String(args.birthday).trim() : undefined;
    if (birthday && !isValidBirthday(birthday)) {
      return JSON.stringify({
        success: false,
        message: "誕生日は YYYY-MM-DD 形式（年不明なら --MM-DD 形式）で指定してください。",
    ...
    const ok = updateContact(ctx.userId, ctx.botId, id, {
      ...
      ...(args.contact_info !== undefined ? { contactInfo: String(args.contact_info).trim() } : {}),
