 ...
 */
function disambiguateFunctionName(
	serverId: number,
	toolName: string,
	used: Set<string>,
): string {
	...
	const hash = createHash("sha1")
		...
		.slice(0, 6);
	let candidate =
		base.slice(0, MAX_FUNCTION_NAME_LENGTH - (hash.length + 1)) + `_${hash}`;
	let n = 0;
...
function jsonSchemaToGeminiSchema(schema: Record<string, unknown>): Schema {
	...
	if (rawType === undefined || rawType === null) {
		...
		if (combinator) {
			...
			const isNullSchema = (s: Record<string, unknown>) =>
		...
		case "object": {
			...
			for (const [key, value] of Object.entries(props)) {
				if (value && typeof value === "object") {
					properties[key] = jsonSchemaToGeminiSchema(
...
async function ensureFreshToolsCache(
	...
	if (cacheAge > TOOLS_CACHE_TTL_MS) {
		try {
			...
			console.warn(
				`[MCP] ${server.name} のToolキャッシュ更新に失敗しました（既存キャッシュで続行）:`,
				err,
			);
		}
	}
	...
	for (const server of servers) {
		...
		for (const tool of tools) {
			...
			if (usedNames.has(fnName)) {
				...
				console.warn(
					`[MCP] ${server.name}: Function名 "${baseName}" が衝突したため "${fnName}" に退避しました (tool: ${tool.name})`,
				);
			}
			...
			const declaration: FunctionDeclaration = {
				name: fnName,
				description:
					`[MCP拡張: ${server.name}] ${baseDescription} ${confirmNote}`
						.trim()
						.slice(0, 1000),
			};
			...
			handlers[fnName] = async (
				...
				if (!available) {
					return JSON.stringify({
						success: false,
						message:
							"このMCPサーバーは現在利用できません（無効化または削除されています）。",
					});
				}
...
export async function getMcpFunctionModuleForBot(
	...
	try {
		...
		console.error(
			`[MCP] Bot ${botId} のサーバー一覧の取得に失敗しました:`,
			err,
		);
		return { declarations: [], handlers: {} };
	}
