...
const declarations: FunctionDeclaration[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        title: { type: SchemaType.STRING, description: "グラフのタイトル（例: '6月のカテゴリ別支出'）" },
        labels: {
          type: SchemaType.ARRAY,
          items: { type: SchemaType.STRING },
          description: "各データ点のラベル（例: ['食費','日用品','娯楽'] や ['1月','2月','3月']）",
        },
        values: {
          type: SchemaType.ARRAY,
        ...
        series_label: { type: SchemaType.STRING, description: "values系列の名前（例: '支出'）（任意）" },
        second_values: {
          type: SchemaType.ARRAY,
          items: { type: SchemaType.NUMBER },
          description: "比較用の第2系列データ（任意。例: 収入系列）。pie/doughnutでは使用不可",
        },
        second_label: { type: SchemaType.STRING, description: "第2系列の名前（例: '収入'）（任意）" },
      },
...
const VALID_TYPES: ChartType[] = ["pie", "doughnut", "bar", "horizontalBar", "line"];

const handlers: FunctionModule["handlers"] = {
  async sendChart(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    // リッチ返信が無効の場合はグラフを生成しない（§3.0.5）
    if (!ctx.richReplyEnabled) {
      return JSON.stringify({
        success: false,
        message: "ユーザー設定によりリッチ返信（グラフ）は無効です。数値はテキストで簡潔に伝えてください。",
      });
    }
    ...
    const values = Array.isArray(args.values) ? args.values.map((v) => Number(v)) : [];

    if (labels.length === 0 || values.length === 0) {
      return JSON.stringify({ success: false, message: "labels と values は必須です。" });
    }
    if (labels.length !== values.length) {
      return JSON.stringify({ success: false, message: "labels と values の件数が一致していません。" });
    }
    if (values.some((v) => !Number.isFinite(v))) {
      return JSON.stringify({ success: false, message: "values に数値でない要素が含まれています。" });
    }
    if (labels.length > 30) {
      return JSON.stringify({ success: false, message: "データ点が多すぎます（最大30件）。集約してから再度呼び出してください。" });
    }

    const datasets: { label?: string; data: number[] }[] = [
      { label: args.series_label ? String(args.series_label) : undefined, data: values },
    ];
    ...
    if (secondValues && secondValues.length > 0) {
      if (type === "pie" || type === "doughnut") {
        return JSON.stringify({ success: false, message: "pie / doughnut では第2系列は使用できません。" });
      }
      if (secondValues.length !== labels.length) {
        return JSON.stringify({ success: false, message: "second_values の件数が labels と一致していません。" });
      }
      datasets.push({
