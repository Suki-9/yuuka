...
const declarations: FunctionDeclaration[] = [
    ...
    parameters: {
      ...
      properties: {
        ...
        remind_before_minutes: {
          ...
          description: "何分前にリマインドするか（省略時はユーザー設定のデフォルト値）",
        },
        ...
        local_only: {
          ...
          description: "Googleカレンダーに同期せず、ボットのローカル通知のみに留めるか",
        },
      },
  ...
  {
    name: "listSchedules",
    description: "今後の予定一覧を取得する（取得前にGoogleカレンダーとの双方向同期を実行する）",
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        days: {
  ...
  {
    name: "deleteSchedule",
    description: "予定を削除する（Googleカレンダーに同期済みの場合はカレンダー側からも削除される）",
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
...
const handlers: FunctionModule["handlers"] = {
  async addSchedule(ctx: ToolContext, rawArgs: Record<string, unknown>): Promise<string> {
    ...
    if (!args.title || !args.start_at) {
      return JSON.stringify({ success: false, message: "title と start_at は必須です。" });
    }
  ...
  async listSchedules(ctx: ToolContext, rawArgs: Record<string, unknown>): Promise<string> {
    const userId = ctx.userId;
  ...
  async deleteSchedule(ctx: ToolContext, rawArgs: Record<string, unknown>): Promise<string> {
    const userId = ctx.userId;
    const scheduleId = Number(rawArgs.schedule_id);
    if (!Number.isInteger(scheduleId)) {
      return JSON.stringify({ success: false, message: "schedule_id が不正です。" });
    }
