...
function displayDateTime(value: string): string {
  const m = value.trim().replace("T", " ").match(/^(\d{4}-\d{2}-\d{2})\s(\d{2}:\d{2})/);
  return m ? `${m[1]} ${m[2]}` : value;
}

/** 送信先の表示ラベル */
function targetLabel(reminder: Pick<ReminderRecord, "target_type" | "target_id">): string {
  if (reminder.target_type === "channel") {
    return reminder.target_id ? `チャンネル <#${reminder.target_id}>` : "チャンネル（既定送信先）";
  }
  return "DM";
}
...
function reminderLine(reminder: ReminderRecord): string {
  const repeat = reminder.repeat_rule ? ` 🔁 繰り返し(${reminder.repeat_rule})` : "";
  return (
...
const declarations: FunctionDeclaration[] = [
    ...
    parameters: {
      ...
      properties: {
        message: {
          ...
          description: "リマインドで通知するメッセージ本文（例: '会議の資料を準備する'）",
        },
        trigger_at: {
          type: SchemaType.STRING,
          description:
    ...
    parameters: {
      ...
      properties: {
        include_all: {
          ...
          description: "送信済み・キャンセル済みのリマインドも含めるか（デフォルト false = 送信待ちのみ）",
        },
      },
    },
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        reminder_id: { type: SchemaType.NUMBER, description: "キャンセルするリマインドのID（#番号）" },
      },
...
const handlers: FunctionModule["handlers"] = {
  ...
  async addReminder(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const message = asOptionalString(args.message);
    if (!message) {
      return JSON.stringify({ success: false, message: "リマインドのメッセージを指定してください。" });
    }
  ...
  async listReminders(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    ...
    if (reminders.length === 0) {
      return JSON.stringify({
        success: true,
        message: includeAll ? "リマインドはありません。" : "送信待ちのリマインドはありません。",
        reminders: [],
      });
    }

    const lines = reminders.map(
      (r) => `${reminderLine(r)}${includeAll ? ` [${statusLabel(r.status)}]` : ""}`
    );
    return JSON.stringify({
      success: true,
  ...
  // リマインドのキャンセル（§3.3.1）
  async cancelReminder(ctx: ToolContext, args: Record<string, unknown>): Promise<string> {
    const reminderId = typeof args.reminder_id === "number" ? args.reminder_id : NaN;
