    ...
    parameters: {
      ...
      properties: {
        keyword: {
          ...
          description: "検索キーワード（例: 'ラーメン', '旅行 計画'）。省略すると期間のみで検索します。",
        },
        from: {
          type: SchemaType.STRING,
          description: "検索期間の開始日 (YYYY-MM-DD形式。例: 先週なら先週月曜の日付)（任意）",
        },
        to: {
          type: SchemaType.STRING,
          description: "検索期間の終了日 (YYYY-MM-DD形式。その日の終わりまで含む)（任意）",
        },
        limit: {
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        keyword: {
          type: SchemaType.STRING,
          description: "要約したい話題のキーワード（例: '旅行', '引っ越し'）。省略すると期間のみで検索します。",
        },
        from: {
          type: SchemaType.STRING,
