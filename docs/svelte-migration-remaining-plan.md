...
対象: `/home/suki/web/kawaii-music.moe/apps/yuuka` / ブランチ `feature/svelte-migration`
親設計書: `/home/suki/web/kawaii-music.moe/apps/yuuka/docs/svelte-migration-plan.md`（全16章。以下 §N はこの親書の章番号を指す）
...
cd /home/suki/web/kawaii-music.moe/apps/yuuka && env -u BASE_URL pnpm dev:host
```
...
cd /home/suki/web/kawaii-music.moe/apps/yuuka && pnpm dev:all
# ホスト直 7854 を使うなら:
...
```bash
cd /home/suki/web/kawaii-music.moe/apps/yuuka
...
#### 完了条件（数値は相対条件を主とする）
```bash
cd /home/suki/web/kawaii-music.moe/apps/yuuka && pnpm build:front
...
削除するファイル（src/public 配下の旧 vanilla フルセット）:
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/app.js
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/styles.css
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/index.html
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/sw.js
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/theme-init.js
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/manifest.json
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/404.html
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/vendor/chart.umd.min.js
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/icons/      （frontend/public/icons へ複製済みを確認後）
  /home/suki/web/kawaii-music.moe/apps/yuuka/src/public/materials/  （同上）
...
- `/home/suki/web/kawaii-music.moe/apps/yuuka/frontend/src/routes/BotShell.svelte`（15静的 import・`TAB_COMPONENTS`・動的 `<Current />` = §P1b 対象）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/frontend/src/routes/tasks/GanttChart.svelte`（`:42` chart.js 動的 import。既に `chart-*.js` へ分離済み）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/frontend/src/App.svelte`（オーバーレイ静的 import・`effectiveView` = §P1b 対象）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/frontend/src/main.ts`（SW 登録未実装 = §P1a 対象）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/frontend/src/vite-env.d.ts`（`vite-plugin-pwa/client` reference 追加先 = §P1a）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/frontend/vite.config.ts`（proxy・outDir・VitePWA 追加先）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/frontend/index.html`（GSVコメント・同期 theme-init.js）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/src/server.ts`（`PUBLIC_DIR:66-68`・immutable `:154-160`/正規表現 `:157`・GSV `:171-185`・CSP `:80-81`（`worker-src 'self'`=`:81`）・method 非依存 writeHead `:198,:222`・gzip 経路 `:105-108` = 不変）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/src/config.ts`（`:7` CONFIG_PATH=cwd/config.yaml・`:98` `port: getSetting('PORT','3000')` = 7854 の由来）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/config.yaml`（`PORT: 7854` = host-direct のポート）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/src/public/sw.js`（旧 v10・固定パス precache = 退役対象）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/Dockerfile`（`:66` --frozen-lockfile / `:68-73` COPY frontend + vite build 挿入 / `:82` prune / `:104-106` COPY 調整）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/docker-compose.dev-hot.yml`（`:4-6,30-32` コメント同期）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/deploy/instance.sh`（`:11` ヘッダ / `:163` usage echo / `:124-125` hot case 本体=不可侵 / `:68-89` verify 強化・`:83` 200 判定・`:84` 成功 echo 置換）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/deploy/dev/instance.env`（HOST_PORT=7855 = dev-hot コンテナ公開ポート）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/package.json`（`:16-20` scripts / `:58-76` devDeps・要 `vite-plugin-pwa` 追加）
- `/home/suki/web/kawaii-music.moe/apps/yuuka/docs/svelte-migration-plan.md`（親書 §5-§7,§13,§14,§15）
